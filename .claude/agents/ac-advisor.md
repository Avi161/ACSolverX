...
- **Benchmarks/data:** `data/ms640_solved.txt` (640 solved MS instances, flat 48-int rows, cap 24); `benchmark/combined/benchmark_combined_{11,22,44,66}.json`; the 66-set is the active CoV benchmark. Equivalence classes: the 261 unsolved MS reps = 168 classes under exact Aut(F₂) canonicalization, ≤ 125 under ACA search so far (`results/equivalence_classes/`).
- Tests: `pytest tests/greedy tests/stable_ac tests/analysis -q` gates every pipeline edit; test budgets ≤ 1,000 by design.
