...
**And it is not an artifact of the benchmark.** The obvious objection to any tuned ordering is that it is the best ordering *that particular training set* could reveal. EXP-27 tested exactly that: 260 fresh configs over all 17 features, selected on 50 presentations no earlier stage had read, then scored on 25 more held back by a split frozen before any fitting. The re-tuned winner reached **19/25** — a dead tie with the recommended ordering — and *lost the half it was selected on* (29/50 against 31/50). A config fitted on entirely different presentations is better on the re-tune's own training data than the re-tune's own winner. On those same 25 rows the baseline gets 7/25 against the recommendation's 19/25.
...
**Memory.** Three modes, all result-pure (identical rows, pinned by `tests/heuristic_search/test_hsolve.py` and `test_hcompact.py`), certificates recovered on any solve by an automatic deterministic re-run. Measured worst-case on the unsolved 124 at cap 48: `hsolve` with the certificate map **36.5 kB per node popped**; `keep_path=False` **24 kB** (10⁶ fits a 51 GB machine); **`hcompact` — the ordering ported onto the compact packed arena — ~78 B per state, ~7 GB reserved at 10⁶, moving the 51 GB ceiling from ~2M to ~5M nodes and running ~13% faster** ([HCOMPACT.md](HCOMPACT.md), an 880-search pop-for-pop cross-check). In the notebook: `ENGINE = "hcompact"`.

**The one prediction worth checking — now settled (EXP-28, the user's Colab run at 100,000 nodes).** The pre-registered gap-vs-budget metric read **turned over** (+14 at 1,000 → +6 at 100,000) — and the row level shows that reading is a saturation artifact: **the recommended ordering finished the benchmark at 62,534 nodes, 60/60 graded rows solved**, its only misses the six open reach problems, so past 62.5k the gap could only compress. The strict superset held at every checkpoint (baseline-only solves: none), the six tuned-exclusive solves are **all of bin 9 — 6/6 against the baseline's 0/6 at 100k** — and against the baseline's historical `nodes_1M` data the hump-band multiplier is **3.4×–23×** (ms634/ms635: 25k tuned vs 574k baseline), with ~1× parity on ms622–ms625 and 250× on ms628/ms633: difficulty *reordered*, deep bins favored. So the advantage is not mere earliness — at 100k the ordering solves second-hump rows the baseline cannot reach at 100k and needs 214k–574k nodes for. Full analysis: [EXP28_colab_scale.md](runs/EXP28_colab_scale.md).
