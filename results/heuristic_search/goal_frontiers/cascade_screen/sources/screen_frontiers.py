...
def main():
    ...
    parser.add_argument('--engine', choices=('frontier', 'cascade'), default='frontier')
    ...
    if args.engine == 'cascade':
        from experiments.search.cascade_heuristics import search as search_call
    ...
    if args.engine == 'cascade':
        paths += [ROOT / 'experiments/search/cascade_heuristics.py', ROOT / 'experiments/search/bs_collapse.py']
    hashes = {}
    for p in paths:
    ...
    if args.engine == 'cascade':
        ...
        for arm in ('s20', 'aut_edges'):
            mixed_search(('xyX','yyx'), arm, budget=5, cap=args.cap)
    for setting in settings.values():
        record = search_call(('YYXyx', 'Yx'), budget=20 if args.engine == 'cascade' else 5, cap=args.cap, **setting)
        if record['solved']:
            assert verify(('YYXyx', 'Yx'), record)
    manifest = dict(settings=settings, engine=args.engine, ids=[r['pres_id'] for r in rows], budget=args.budget,
    ...
    for i, row in enumerate(rows):
        ...
        for name in names[shift:]+names[:shift]:
            ...
            result = search_call((row['r1'], row['r2']), budget=args.budget, cap=args.cap, **settings[name])
