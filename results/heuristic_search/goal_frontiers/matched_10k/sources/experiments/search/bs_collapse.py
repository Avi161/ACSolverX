...
def bs_collapse(pair, budget=10_000, intermediate_cap=256):
    ...
    if isinstance(intermediate_cap, bool) or not isinstance(intermediate_cap, int) \
            or not 1 <= intermediate_cap <= 256:
        raise ValueError("intermediate_cap must be an integer in 1..256")
    ...
    def rewrite(word, donor, lhs, rhs, position=0):
        ...
        if max(map(len, desired_pair)) > intermediate_cap:
