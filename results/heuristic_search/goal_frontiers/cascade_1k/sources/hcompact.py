...
search is deterministic, so the path is exact.
    ...
    stats = greedy_search_hcompact(r1, r2, node_budget=3 * 10**6,
                                   max_relator_length=48, config=S20_MK2)
"""
import os
import sys
...
from experiments.heuristic_search.core.hsolve import LENGTH_ONLY, S20_MK2         # noqa: E402
...
class HCompactSolver:
    ...
    def __init__(self, r1, r2, max_nodes=10000, max_relator_length=24,
        ...
        if self.track_path and self.cap > 128:
            raise ValueError("path capture requires max_relator_length <= 128 (int8 cuts)")
        ...
        if max(len(a1), len(a2)) > self.cap:
            raise ValueError("reduced initial relator exceeds max_relator_length")
    ...
    def _alloc(self, n, old=None):
        if n > np.iinfo(np.int32).max:
            raise MemoryError("compact state capacity exceeds signed int32 IDs")
        self.states_cap = n
