...
[`ABEL_TIEBREAK_B1K.md`](comparison/ABEL_TIEBREAK_B1K.md) + `abel_tiebreak_b1k_subset60.csv` ask which key the abelianized-magnitude rule wants *second*, and answer it at **zero search nodes** by re-ranking the same frozen sweep through `abel_topk_cov_b1k`'s gated loader (`experiments/heuristic_search/runners/abel_tiebreak_b1k.py`). Because a key costs nothing to evaluate, **every one of `hlab`'s 17 features is scored in both positions** — `(abel, f, total)` and `(abel, total, f)` — plus the full `RECOMMENDED` linear score as a single key. The result is one-sided: **not one feature placed ahead of total length beats it**, 9 of 18 lose rank-1 solves and `Bmax`/`xyimb`/`Bmaxrun`/`Bspread` collapse from 40/60 to 22/60; placed *behind* length, 12 of 18 land within 6 nodes of `(abel, total)`'s 61,627 and the rest give a rank-1 solve back. That includes `S` (smallest mean block), the heaviest weight in `hsolve.RECOMMENDED` at 8.458 and the `S` in `L + 20·S + 2·MK` — as a lexicographic **start** key `(abel, S, total)` costs 2 rank-1 solves and 47% more nodes than `(abel, total, S)`. The mechanism is the file's most transferable result: **`S` never ships alone.** `s20_mk2` and `RECOMMENDED` both keep `L` inside the same expression, where it can outvote `S`, and a lexicographic first key cannot be outvoted — score the composite itself and `(abel, L + 20·S + 2·MK)` is the joint-cheapest arm at 61,625 nodes, 2 off `(abel, total)`. So the "before length" column is not evidence that these features are bad, only that lexicographic priority is the wrong way to spend them; none of it bears on `S` as the **climb** feature it was tuned to be. The chain that discriminates *most* (`abel + S + total`, 48/60 unique picks) is one of the worst performers, so tie-breaking power is not the objective. Three side results: "mean relator length" is not a distinct key at all (two relators ⇒ mean = total/2, proved on all 60 by `assert_mean_is_length`); `min(|r1|,|r2|)` and the total are a **dead heat** (1 of 41 both-solved rows, 2 nodes, at both budgets); and the +2-row rank-1 margin any length key buys at budget 1,000 **inverts at 10,000**, so subset-60 cannot separate these keys — the powered evidence is the ms640 census, not this file. It also settles what to do when the tie survives `(abel, total)`: **nothing.** 29 of 60 are still tied, but their 102 tied candidates are only 52 distinct Booth-canonical pairs (49% are literally the same search) and 28 of the 29 tied sets are homogeneous — every member solves, or none does — so the pick cannot matter on any of them but one (533). Applying the [`cov_top3`](stable_ac/cov/cov_top3/RESULTS.md) canonical dedup before the top k changes the top-3 set on 42 of 60 rows and moves **zero** nodes and zero solves at both budgets; that is the abel-arm half of a recommendation whose measured waste was always a `len`-arm problem (325,963 nodes against abel's 707), so keep the dedup as hygiene and never quote it as a gain. The remaining headroom is not in the tie-break and not in a wider k (k=3→5 buys 0): the 4 rows below the oracle have their solving candidate at rank 8, 8, 49, 44, solving in 410–632 nodes, and all four sit **one abel step above the minimum shell** — a primary-ranking failure. Three class-quota policies fail to recover any of them. The held-out 124 is unusable here: 0 of its 124 has any solving CoV candidate at budget 50,000.
