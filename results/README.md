...
[`ABEL_TIEBREAK_B1K.md`](comparison/ABEL_TIEBREAK_B1K.md) + `abel_tiebreak_b1k_subset60.csv` ask which key the abelianized-magnitude rule wants *second* — the shorter relator or the shorter total — and answer it at **zero search nodes**, by re-ranking the same frozen sweep through `abel_topk_cov_b1k`'s gated loader (`experiments/heuristic_search/runners/abel_tiebreak_b1k.py`). Three results worth carrying forward: "mean relator length" is not a distinct key at all (two relators ⇒ mean = total/2, proved on all 60 by `assert_mean_is_length`, not argued); `min(|r1|,|r2|)` and the total are a **dead heat** as the tie-break (1 of 41 both-solved rows, 2 nodes, at both budgets), and min discriminates strictly less on its own (unique pick on 24/60 against 31/60); and the +2-row rank-1 margin any length key buys at budget 1,000 **inverts at 10,000** (−1 row, higher mean), so subset-60 cannot separate these keys — the powered evidence is the ms640 census, not this file. The held-out 124 is unusable here: 0 of its 124 has any solving CoV candidate at budget 50,000.
