...
**Result 98 (the two-turn separated-return stratum is obstructed).**
Let
...
where \(4\nmid r,u\), \(3\nmid t,v\), and \(\ell,s,q\) are
arbitrary.  Over \(\overline{\mathbb F}_5\), choose fresh
\(w,p\in V_1\), disjoint complete fourth-phase packets
\(A,c\in D_1\), and disjoint cube-phase packets
...
The source list is independent because both fourth-phase pairs have
rank two.  The image list is independent because the two cube-phase
packets are disjoint and have determinants
...
The partial map therefore extends globally.  It satisfies
\(wYR_3=4w=wR_4\), and

\[
wb=QX^tY^{-1}X^uYX^vY^{-1}X^q
=4hX^{-v}X^vY^{-1}X^q
=4w.
\]
...
**Result 102 (the penultimate positive-start length-six stratum is
obstructed).**  Let
...
\(\ell\not\equiv0\pmod4\), choose mixed \(w\in D_1\) so that
\(w,wX^\ell,S\) are independent, and choose \(hR_3=S\).
Using mutually disjoint packets, choose \(P,C\in\mathcal R_1\),
\(A\in D_1\), and \(B,D\in V_1\) so that

\[
P,PX^r,\qquad A,AX^s,\qquad C,CX^u
\]

are independent pairs.  Prescribe
...
\[
BY=C,\qquad DY=CX^u,\qquad SY=D.
\]

Both finite lists are independent, so the prescription extends.
The resulting word replay is
...
Then \(hR_3=S\), while \(h,hX^r\) are independent because
\(3\nmid r\).  With fresh \(A,C,B,D\) as above, prescribe

\[
wY=h,\quad AY=hX^r,\quad (AX^s)Y=B,\quad
BY=C,\quad DY=CX^u,\quad SY=D.
\]

Again both lists are independent and the same replay ends at \(S\).
Thus
...
in every residue case.  The Fox bridge and Result 85 close both A--D
signs for every free-kernel lift.  The complete
\((+,-,+,+,-,-)\) stratum is obstructed, leaving only
...
**Result 117 (a genuine two-source target forces two proper relative
torus extensions).**  Consider
...
\(F_0*\langle r\rangle\).  If \(W\) is primitive, its quotient
\(Q\cong F_2\) again contains the old factor \(K\cong F_0\), with
\(H_1(K)\cong H_1(Q)\).  Write \(a,b\) for the images of \(A,B\).

Killing \(a\) gives the exact relative presentation

\[
P_A=Q/\langle\!\langle a\rangle\!\rangle
\cong
\langle G_{3,4},r
\mid r d_A B_A^\eta d_A^{-1}\rangle,
\]

where \(d_A\) is the image of the full r-dependent conjugator.  Its
relative relator has r-exponent one.  Klyachko's unimodular-equation
theorem injects the torsion-free coefficient group

\[
G_{3,4}\hookrightarrow P_A.
\]

This inclusion is an isomorphism on first homology, and the image of
\(B\) normally generates all of \(P_A\).  Symmetrically,

\[
P_B=Q/\langle\!\langle b\rangle\!\rangle
\cong
\langle G_{2,3},r
\mid r c_B A_B^\epsilon c_B^{-1}\rangle
\]

contains \(G_{2,3}\), again homologically onto, and the image of \(A\)
normally generates \(P_B\).

If either coefficient inclusion is onto, Collins--Zieschang and Turner
make \(K\hookrightarrow Q\) onto.  Magnus rigidity then forces
...
Both proper extensions must occur simultaneously.  Combining the two
source factors into
...
gives another exact quotient reduction, but it exposes a marked-relator
mismatch: the relator killed in \(Q\) is the image of \(Z\), whereas the
relator killed in \(K\) is the image obtained after first setting \(r=1\).
They need not agree, so Turner cannot be applied to one fixed word.

The condition \(\lambda(W)>2\) is essential.  If both conjugators lie in
\(F_0\), the word is \(rU\) and primitive; even \(c=1,d=r\) cyclically
reduces to a one-r word of syllable length two.  The live two-source
...
**Result 138 (the hardest overlap has no axial solution).**  For the
signature \((8,3,5,-3,5)\), use the target-adapted free basis

\[
c=yyX,\qquad t=c^3(yX)=\operatorname{Chr}(-4,7).
\]

Let \(K=\ker(\chi(c)=1,\chi(t)=0)\), freely generated by
\(t_j=c^jtc^{-j}\).  Exact row push-through identities and four Bezout
normalizations move every hypothetical solution to conjugators in
\(K\).  In Schreier coordinates the remaining equation has shift gaps
\(3,2,1,1\), and its target condition is exactly that the final cyclic
kernel word be one positive basis letter \(t_m\).

When the original conjugators are all powers of \(c\), the kernel word
is an eight-block word of length 43.  Complete suffix--prefix comparison
shows that only three local cancellation types can occur, and the five
resulting affine conditions permit at most nine cancellation pairs.
Thus its cyclic length is at least \(43-18=25\), with equality attained,
so this entire axial stratum misses the one-letter target.

The restriction is essential.  The Alexander module of the cyclic
cover is blind because the final seam can repair every discrepancy of
augmentation one.  Result 138 therefore reduces, but does not close,
this depth-four class: only genuinely nonabelian kernel order can now
...
**Result 146 (the binomial lifting operators form a forest).**  The
incidence graph of
\[
L_2=1-X,\qquad L_3=U^{-1}-t,\qquad L_4=t-1
\]
on \(Q/\langle c\rangle\) is generated by \(t,X,G=U^{-1}t^{-1}\).
In \(Q=F(p,q)\rtimes C_2\),
\[
t=p,\qquad X=(qPQp)c,\qquad G=(qPPq)c.
\]
The five Reidemeister--Schreier generators of the parity kernel fold to a
Stallings core with four vertices, eight unoriented edges, and rank five.
Hence \(\langle t,X,G\rangle\cong F_3\) is torsion-free and acts freely on
every \(C\)-vertex orbit.  The incidence graph is a disjoint union of
trees, so
\[
L_2y_2+L_3y_3+L_4y_4=0
\]
has no nonzero finitely supported solution.  Every remote homogeneous
lifting syzygy must use \(L_0\) or \(L_1\).

**Result 147 (an exact remote syzygy clears two bits but meets a four-point
obstruction).**  A \(23\)-entry integral vector \(w\), with coefficient
\(\ell^1\)-norm \(35\), satisfies
\[
\sum_{i=0}^4L_iw_i=0.
\]
It has exactly four entries outside the one-hop set.  Adding it to the
shortest first-layer lift gives a nonlinear residual of free length \(476\),
kernel length \(124\), and degree-two wedge support \(170\), with
\[
(\Phi_\infty,\Phi_3)=(0,0).
\]
Thus nonlocal support really can defeat both obstructions from Result 145.

The four-point action
\[
c=(2\ 3),\qquad t=(0\ 1\ 2)
\]
maps this new residual to the six wedge coordinates
\[
(-5,4,-36,17,15,-18).
\]
Over \(\mathbb F_2\), the induced operator image has rank five and adjoining
this vector raises the rank to six.  Hence the remote lift is still
degree-two obstructed.  The next syzygy must involve \(L_0\) or \(L_1\)
and must also kill this four-point parity.

**Result 148 (a source-coupled syzygy kills all three recorded bits).**
There is a 16-entry homogeneous syzygy \(z\), of coefficient
\(\ell^1\)-norm 16, whose source component is
\[
z_1=e_{ct}.
\]
The \(L_1\)-source column has coefficient sum zero on each of the two
\(\langle t,X,U^{-1}t^{-1}\rangle\)-orbits.  The binomial-forest theorem
therefore supplies its unique finite flow.  Two exact Cayley-tree paths,
with basis \(A=t,B=X,G=U^{-1}t^{-1}\), are
\[
...
Adding \(z\) directly to the canonical eight-entry lift gives a nonlinear
residual of free length 322, kernel length 70, and degree-two wedge support
112.  Its full wedge sum is \(-10\); its three-point coordinates are
\((-1,-21,-10)\); and its four-point coordinates are
\((6,6,5,6,-3,-4)\).  All three coordinate sums are even, so
\[
(\Phi_\infty,\Phi_3,\Phi_4)=(0,0,0).
\]
Hence these three functionals are not a global obstruction on compactly
supported first-layer solutions.  This does not decide global image
membership of the complete 112-term residual in
\(\sum_iL_i(\Lambda^2M)\); that exact problem is the next frontier.

**Result 149 (a cyclic three-point quotient obstructs the source-coupled
lift).**  Over \(\mathbb F_2\), use the three-point action
\[
c=1,\qquad t=(0\ 1\ 2).
\]
The coordinate-sum functional on
\(\Lambda^2\mathbb F_2[\{0,1,2\}]\) is \(Q\)-invariant.  Since every
\(L_i\) has augmentation zero, it annihilates the entire second-layer image
\(\sum_iL_i(\Lambda^2M)\), without a support bound.

The 112-term residual from Result 148 maps exactly to
\[
(-1,-3,-3),
\]
whose sum is odd.  Hence that first-layer solution does not lift through
degree two.  The induced operator image has rank two over \(\mathbb F_2\),
and adjoining the residual raises the rank to three.

Moreover the four functionals
\(\Phi_\infty,\Phi_3,\Phi_4,\Psi_{\rm cyc}\) separate all 16 nonlinear
replays in the mod-two affine span generated by the two one-hop directions
and the two certified remote syzygies.  This is not a global theorem about
...
**Result 155 (the inverse four-cycle closes the known nine-direction span).**
There is a 43-entry, coefficient-norm-58 homogeneous syzygy $j$ whose
$L_0$-source is
\[
j_0=e_{TT}+e_{tcTct}.
\]
Six exact Stallings-cover paths reconstruct its edge flow and prove
\(\sum_iL_ij_i=0\).  It raises the known homogeneous-direction rank from
eight to nine modulo two.  Its residual has free length 1578, kernel length
262, wedge support 535, and kills all ten Result 154 mod-two bits.

The inverse four-cycle action
\[
c=(1\ 2),\qquad t=(0\ 3\ 2\ 1)
\]
has operator-image rank five over \(\mathbb F_2\).  The all-ones wedge
covector annihilates the image, while the ninth residual maps to
\[
(26,-8,-18,-4,-3,4),
\]
whose coordinate sum is odd.  Hence adjoining the defect raises the rank
to six.

The resulting eleven bits are integer-valued quadratic functions of nine
coefficients.  Fifty-five exact replays determine the complete model, 45
further points validate it, and all \(4^9=262144\) mod-four classes have
nonzero obstruction vector.  Therefore the complete known nine-direction
...
abstract presentation of the same group. Result 117 treats two sources
multiplied directly into the primitive slot. Klyachko embeds both residual
