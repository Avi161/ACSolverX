...
replacements: the conditional transfer below needs a separate projected-pair
AC certificate. Changing the target rows, the unrestricted MMS02 bridge,
and stable or ordinary AK3 remain
...
**Conditional two-complement transfer and one closed completion.** Let
\((A,B)\) be a balanced trivial presentation and let
\(f:F(r,s,t,u)\twoheadrightarrow F(x,y)\) send \(r,s\) to \(A,B\).
Suppose an explicit free basis \((W_1,W_2,X_0,Y_0)\) satisfies
\(f(W_i)=1,f(X_0)=x,f(Y_0)=y\). Let \(\pi\) kill r,s and put
\(p_i=\pi(W_i)\). The pair \((p_1,p_2)\) normally generates
\(F(t,u)\): its quotient is isomorphic to
\(F(x,y)/\langle\!\langle A,B\rangle\!\rangle=1\), since the
kernel of f is normally generated by \(W_1,W_2\). Unimodularity alone
would not prove this assertion.

If \((p_1,p_2)\) is **ordinary AC-trivial**, reverse its tag-only
trivialization to take the basis rows \((r,s,t,u)\) to
\((r,s,p_1,p_2)\). Set \(h_1=r^{-1}W_1,h_2=s^{-1}W_2\).
Because \(\pi(h_i)=p_i\), restored r,s donations correct the tags
to \(h_1,h_2\); right multiplication then changes r,s to \(W_1,W_2\).
Transport this entire basis-started path through the basis map
\((W_1,W_2,X_0,Y_0)\mapsto(t,u,x,y)\), prefixing Nielsen row moves
to its transformed starting basis. At the endpoint the actual t,u rows
clear all t,u letters from the remaining rows and can be deleted;
the survivors are \(A^{-1},B^{-1}\). Reversing this construction gives
a stable trivialization using two extra generator-relator pairs. This
is conditional sufficiency, not an unconditional two-complement criterion.

For the single completion \(R=x^3y^{-4},S=xyxYXY\), take
\(f(t)=xyxy,f(u)=xyx\). Then \(X_0=tU s,Y_0=Ut\), with inverse
\(t=uY_0,s=uY_0^{-1}u^{-1}X_0\), proves that
\((X_0,Y_0,r,u)\) is a free basis. Set
\(W_1=rR(X_0,Y_0)^{-1}\) and
\(W_2=u(X_0Y_0X_0)^{-1}\). The inverse formulas
\(r=W_1R(X_0,Y_0),u=W_2X_0Y_0X_0\), followed by the preceding
t,s formulas, prove the full claimed basis. After projection, the
coordinates \(a=tU,b=u\) and inversion of the first row give
\(P=a^3BA^4b,Q=bABAbA\).

The ambient coordinate change \(c=ba^{-2}\), hence \(b=ca^2\),
gives \(P=aCA^4ca^2,Q=cACAca\). Inverting Q and conjugating it
by ca gives the actual donor \(S_0=acaCAC\). Put
\(f_0=CAc,g_0=aCA\); literally
\(f_0g_0^{-1}={}^{CA}S_0^{-1}\). Conjugating P by a gives
\(a^2f_0^4a\). Four positive left donations from the current
\(S_0\) row, conjugated by \(a^2g_0^iCA\) for \(i=0,1,2,3\)
and restored each time, give \((a^3C^4,S_0)\).
The [independent literal replay](../../tests/stable_ac/test_ak3_two_complement_return.py)
checks both basis inverses, projections, every donor restoration and
intermediate word, and a wrong-sign control; these word-conjugation
macros carry no primitive-move count.

The ambient coordinate changes above are not asserted to be elementary
relator moves. Rather, the projected pair is ordinary AC-trivial **if
and only if standard AK3 is**: a trivializing sequence transports
through either free-basis map, and its terminal free basis Nielsen-reduces.
Thus this completion does not simplify the conditional sufficient
hypothesis. This closes only this candidate, proves no triviality or
exclusion of other completions, and opens no complement census.
