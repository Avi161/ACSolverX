...
every literal lift over the one fixed quotient witness $\mathbf h^*$, but
the equal-length hyperbolic quotient-solution branch is not classified.
Promoting a fixed-fiber obstruction to the full recurrence requires both
quotient gauge-orbit completeness and lift-compatible gauge invariance.
...
equivalence relation generated by these centralizer gauges.  The available
quotient theorem proves that $\mathscr S_Q$ is nonempty and that
$\mathbf h^*$ lies in the equal-length hyperbolic branch.  It does not
classify that branch.  In particular, neither of the two promotion gates

\[
 \begin{aligned}
 \text{Q-Class: }&\mathscr S_Q=[\mathbf h^*]_{\sim_{\rm rg}},\\
 \text{Q-Lift: }&\text{literal liftability is invariant under }
 \sim_{\rm rg}
 \end{aligned}
 \tag{1.10d}
\]

has been proved.  A broader lift-preserving gauge could replace
$\sim_{\rm rg}$, but its orbit completeness and lift compatibility would
still require proofs.  This is the exact quotient-scope gap.
...
Only after Q-Class and Q-Lift are established would
$\mathsf T_{\rm fixed}$ obstruct the full period-two recurrence attached
...
2. **Literal higher lifting:** if a class-two candidate survives, decide
   ...
   $F(c,t)$.
3. **Quotient coverage:** prove Q-Class and Q-Lift before transferring a
   fixed-witness result to the full signature recurrence.
