...
literal-lift compatible, so the fixed-fiber theorem transfers across their
whole orbit.  The equal-length hyperbolic quotient-solution branch is still
not classified modulo the enlarged action, and quotient centralizer gauges
are not yet proved lift compatible.
...
Let $\sim_{H{\rm rg}}$ be generated by $\Gamma_H$ and the row gauges.  The
following is one sufficient orbit-promotion route:

\[
 \begin{aligned}
 \text{Hrg-Class: }&\mathscr S_Q=[\mathbf h^*]_{\sim_{H{\rm rg}}},\\
 \text{RG-Lift: }&\text{literal liftability is invariant under }
 \sim_{\rm rg}.
 \end{aligned}
 \tag{1.10d}
\]

Neither is proved.  They are not logically necessary: a different
