...
Section 1.1 makes the quotient scope explicit.  Equation (1.10) covers
every literal lift over the one fixed quotient witness $\mathbf h^*$.
The original row-centralizer gauge is now proved incomplete: one exact
Hurwitz symmetry already carries $\mathbf h^*$ through infinitely many of
...
Section 3.106 contracts the complete same-coordinate occurrence chain
...
### 1.1 Scope: one fixed quotient-solution fiber

Let $\pi:F(c,t)\to Q$ be the quotient map.  To distinguish the lifting
problem above from classification of the quotient recurrence, define
$\mathscr S_Q$ to be the set of tuples
$\mathbf g=(g_0,g_1,g_2,g_3,g_4)\in Q^5$ for which

\[
 \begin{aligned}
 R_{\mathbf g}&=Ag_0B^{-1}g_0^{-1},\\
 S_{\mathbf g}&=Bg_1R_{\mathbf g}^{-1}g_1^{-1},\\
 U_{\mathbf g}&=R_{\mathbf g}g_2S_{\mathbf g}^{-1}g_2^{-1},\\
 Z_{\mathbf g}&=U_{\mathbf g}^{-1}g_3S_{\mathbf g}g_3^{-1},\\
 Z_{\mathbf g}&=g_4tg_4^{-1}.
 \end{aligned}
 \tag{1.10a}
\]

The exact quotient witness of Section 1 is

\[
 \mathbf h^*:=(h_0,h_1,h_2,h_3,1)\in\mathscr S_Q.
 \tag{1.10b}
\]

Equations (1.3)--(1.10) quantify over every free-group candidate whose five
quotient conjugators are exactly $\mathbf h^*$ and whose module classes
pass the necessary first-layer equation (1.2).  Indeed, every solution of
that equation is $x^{00}+F$ by (1.3), every lift of such a module class has
the unique form (1.6), and the factors $k_r\in[N,N]$ supply every lift
inside that module fiber.  A candidate failing (1.2) cannot be a literal
solution.  Thus (1.10) is the complete literal lift problem over
$\mathbf h^*$, not a quantifier over all of $\mathscr S_Q$.

There is an automatic row-preserving quotient gauge.  For a solution
$\mathbf g$, right multiplication by

\[
 \begin{aligned}
 z_0&\in C_Q(B),&
 z_1&\in C_Q(R_{\mathbf g}),&
 z_2&\in C_Q(S_{\mathbf g}),\\
 z_3&\in C_Q(S_{\mathbf g}),&
 z_4&\in C_Q(t)
 \end{aligned}
 \tag{1.10c}
\]

leaves every row in (1.10a) unchanged.  Let $\sim_{\rm rg}$ be the
equivalence relation generated by these centralizer gauges.  The row-chain
map

\[
 \rho(\mathbf g)=(R_{\mathbf g},S_{\mathbf g},U_{\mathbf g},Z_{\mathbf g})
 \tag{1.10c1}
\]

classifies this gauge exactly on its image.  Indeed, the gauges in (1.10c)
leave $\rho$ fixed.  Conversely, if two solutions have the same four rows,
equality of their conjugates says successively that the quotients of their
five conjugators lie in
$C_Q(B),C_Q(R),C_Q(S),C_Q(S),C_Q(t)$.  Hence

\[
 \mathbf g\sim_{\rm rg}\mathbf g'
 \Longleftrightarrow
 \rho(\mathbf g)=\rho(\mathbf g').
 \tag{1.10c2}
\]

The recurrence has four larger prefix-Hurwitz symmetries.  Define

\[
 \begin{aligned}
 \mathcal H_A(\mathbf g)
  &=(A^{-1}g_0,g_1A,A^{-1}g_2,A^{-1}g_3,A^{-1}g_4),\\
 \mathcal H_B(\mathbf g)
  &=(g_0,B^{-1}g_1,g_2B,g_3B,g_4),\\
 \mathcal H_R(\mathbf g)
  &=(g_0,g_1,R_{\mathbf g}^{-1}g_2,
     R_{\mathbf g}^{-1}g_3,R_{\mathbf g}^{-1}g_4),\\
 \mathcal H_U(\mathbf g)
  &=(g_0,g_1,g_2,U_{\mathbf g}g_3,U_{\mathbf g}g_4).
 \end{aligned}
 \tag{1.10c3}
\]

Direct substitution in (1.10a) gives

\[
 \begin{array}{c|cccc}
 &R&S&U&Z\\ \hline
 \mathcal H_A&A^{-1}RA&S&A^{-1}UA&A^{-1}ZA\\
 \mathcal H_B&R&B^{-1}SB&U&Z\\
 \mathcal H_R&R&S&R^{-1}UR&R^{-1}ZR\\
 \mathcal H_U&R&S&U&UZU^{-1}.
 \end{array}
 \tag{1.10c4}
\]

The transformed fifth conjugator gives exactly the displayed conjugate of
$t$, so every map in (1.10c3) preserves $\mathscr S_Q$.  Each map is
invertible using the same fixed row in the reverse direction.  Let
$\Gamma_H$ be the group they generate.

This action is not contained in the row gauge.  Iterating the first map
gives

\[
 \mathcal H_A^n(\mathbf g)
 =(A^{-n}g_0,g_1A^n,A^{-n}g_2,A^{-n}g_3,A^{-n}g_4)
 \tag{1.10c5}
\]

and changes $R$ to $A^{-n}RA^n$.  At the fixed witness the reduced words
are

```text
A          = TTcTTcttc
R          = TTctcTctc
A^-1 R A   = cTTctttcTctcTTcTTcttc
```

Thus $A^{-1}RA\ne R$.  By (1.10c2), $\mathcal H_A(\mathbf h^*)$ is not
row-gauge equivalent to $\mathbf h^*$.  The centralizer of the hyperbolic
word $R$ in $C_2*\mathbb Z$ is cyclic.  If two conjugates
$A^{-n}RA^n$ and $A^{-m}RA^m$ agreed, a nonzero power of the hyperbolic
word $A$ would centralize $R$.  The cyclic centralizer of that power would
then contain both $A$ and $R$, forcing them to commute.  The displayed
strict inequality excludes this.  Therefore

\[
 \boxed{
 \mathscr S_Q/{\sim_{\rm rg}}\text{ contains infinitely many classes in }
 \Gamma_H\mathbf h^*.}
 \tag{1.10c6}
\]

In particular, the former assertion
$\mathscr S_Q=[\mathbf h^*]_{\sim_{\rm rg}}$ is false.

Unlike the row-centralizer gauge, the Hurwitz action is already proved
literal-lift compatible.  Apply the formulas in (1.10c3) to arbitrary
free-group lifts of the five conjugators and recompute the recurrence
without using $c^2=1$.  If $\mathcal R$ denotes the literal residual, then

\[
 \begin{aligned}
 \mathcal R_{\mathcal H_A}&=A^{-1}\mathcal R A,&
 \mathcal R_{\mathcal H_B}&=\mathcal R,\\
 \mathcal R_{\mathcal H_R}&=R^{-1}\mathcal R R,&
 \mathcal R_{\mathcal H_U}&=U\mathcal R U^{-1}.
 \end{aligned}
 \tag{1.10c7}
\]

The same identities for the inverse transformations prove equivalence, not
only preservation, of literal liftability throughout every $\Gamma_H$
orbit.  Thus $\mathsf T_{\rm fixed}$ automatically obstructs literal lifts
over all quotient fibers in $\Gamma_H\mathbf h^*$.

Let $\sim_{H{\rm rg}}$ be generated by $\Gamma_H$ and the row gauges.
On the orbit of the fixed witness, the apparent extra gauge-lifting
obligation can be discharged.

**Lemma (root-free gauge rows).** At $\mathbf h^*$ the quotient rows
$B,R,S$ are hyperbolic and are not proper powers. Their centralizers are
therefore $\langle B\rangle$, $\langle R\rangle$, and $\langle S\rangle$.
Also $C_Q(t)=\langle t\rangle$.

**Proof.** The pinned words are
...
Both $B$ and $R$ have $t$-height $-1$, excluding any proper power by
the homomorphism $Q\to\mathbb Z$. Their cyclic free-product normal forms
have more than one syllable. Conjugating $S$ by $t^3$ gives the cyclically
reduced word `cttcTcT`. It has three $c$-syllables and successive nonzero
$t$-exponents $(2,-1,-1)$. If it were a proper power, its hyperbolic
cyclic root would repeat an alternating syllable block; the exponent
would divide three and hence be three. That would require the three
$t$-exponents to agree, which they do not. The cyclic centralizer theorem
for hyperbolic elements of $C_2*\mathbb Z$ now applies. The element $t$
lies in the infinite cyclic free factor, whose malnormality instead gives
$C_Q(t)=\langle t\rangle$. $\square$

The Hurwitz row table (1.10c4) preserves the conjugacy classes of $B,R,S$;
row gauges preserve the rows themselves. Thus these centralizer formulas
hold throughout $[\mathbf h^*]_{\sim_{H{\rm rg}}}$.

**Proposition (literal gauge lifting on the witness orbit).** Literal
liftability is invariant on the whole orbit
$[\mathbf h^*]_{\sim_{H{\rm rg}}}$.

**Proof.** Consider any free-group lift of a conjugator tuple in this
orbit, and let $\widetilde R,\widetilde S$ be its actual recomputed rows.
Every permitted quotient row gauge has the form
$(B^{m_0},R^{m_1},S^{m_2},S^{m_3},t^{m_4})$ for integers $m_i$.
Right-multiply the five literal conjugators by

\[
(B^{m_0},\widetilde R^{m_1},\widetilde S^{m_2},
  \widetilde S^{m_3},t^{m_4}),
\tag{1.10c8}
\]

where $B$ in (1.10c8) is the original free word, not a chosen root of
its quotient image. The first multiplier commutes literally with the
first conjugated donor and leaves $\widetilde R$ unchanged. The second
likewise leaves $\widetilde S$ unchanged. The next two leave the third
and fourth rows unchanged, and the last leaves the target unchanged.
Consequently the entire literal residual is unchanged, even for a lift
that is not a solution. Negative exponents give the inverse gauge lift.
Combine this with (1.10c7) and its inverse identities along an arbitrary
finite word in the Hurwitz maps and row gauges. This gives equivalence
of solution existence over every fiber in the stated orbit. $\square$

The tests independently check the quotient normal forms and the root
syllable data, and replay (1.10c8) on perturbed literal conjugators with a
...
\[
 ...
 \mathsf T_{\rm fixed}:\\
 \mathcal R(F;\mathbf k)\ne1
 \text{ for every }F\in\mathcal H_{\rm fin}
 \text{ and }\mathbf k\in[N,N]^5.
 \end{gathered}
 }
 \tag{1.10e}
\]
...
### 1.2 Strict gate separation

The remaining implications are kept as separate gates:

1. **Class-two closure:** decide (3.8) for every balanced finite $F$ over
   the fixed witness.
...
4. **MMS02 bridge:** decide the independent equivalence from
   $T_{\rm pub}=(A,B,Xyz)$ to the verified trivial descendant
   $(A,B,zYX)$, stably or without stabilization as explicitly claimed.
5. **Stable AK(3):** requires a verified stable bridge from AK(3) to a
   trivial presentation; it is not supplied by any open class-two or MMS02
   gate.
6. **Ordinary AK(3):** requires an unstabilized AC path or an equivalent
   theorem and is strictly stronger than the stable gate.

No gate is treated as completed because a later or earlier one is open,
and no bounded failure is evidence against any of them.
...
### 3.27 Explicit cross-support diagonal cancellation

There is a universal cross-support mechanism which can cancel one diagonal
coefficient of (3.175).  For \(x\in X\), choose either representative
\(\widetilde x\) and put

\[
 r_x:=\widetilde x c\widetilde x^{-1},
 \qquad
 \Phi_I(x):=
 g_I^{-1}r_xg_I\widetilde xH.
 \tag{3.176}
\]

This is well defined: replacing \(\widetilde x\) by
\(\widetilde xc\) leaves \(r_x\) unchanged and does not change the final
right \(H\)-coset.  It has no fixed vertex.  Indeed,
\(\Phi_I(x)=x\) would imply

\[
 (\widetilde x^{-1}g_I\widetilde x)^{-1}
 c(\widetilde x^{-1}g_I\widetilde x)\in H.
\]

Malnormality of \(H\) would then put the infinite-order conjugate
\(\widetilde x^{-1}g_I\widetilde x\) in \(H\), a contradiction.  For
the six nonzero-height generators, \(\Phi_I(x)\) also lies on a different
\(g_I\)-axis: equality with \(g_I^kxH\) forces \(k=0\) by height and is
then the excluded fixed-point case.

If \(y=\Phi_I(x)\), then

\[
\boxed{
 H\widetilde x^{-1}g_I\widetilde yH
 =Hc\widetilde x^{-1}g_I\widetilde xH
 =\mathscr D_I(x).}
\tag{3.177}
\]

Thus the ordered cross pair \((x,y)\) hits exactly the same oriented
bucket as the diagonal pair \((x,x)\).  The map also preserves height:
...
because \(\nu_t(r_x)=0\).  Hence the height filter alone cannot make this
cross-incidence triangular.

The cancellation is already visible on the nonzero two-atom current
\(f=e_x-e_y\).  In \(-f\otimes g_If\), the four ordered coefficients are

\[
\begin{array}{c|c|c}
(x,x)&-1&\mathscr D_I(x)\\
(x,y)&+1&\mathscr D_I(x)\\
(y,x)&+1&H\widetilde y^{-1}g_I\widetilde xH\\
(y,y)&-1&\mathscr D_I(y).
\end{array}
\tag{3.179}
\]

The first two rows cancel integrally.  After projection to the complete
conjugate-\(g_I\) axis family, the honest remainder is therefore

\[
 \boxed{
 \Pi_I\!\left(-f\otimes g_If\right)
 =\Pi_I\!\left(
   [H\widetilde y^{-1}g_I\widetilde xH]
  \right)-\delta_{\mathscr D_I(y)}.}
 \tag{3.180}
\]

Here \(\Pi_I\) retains the appropriate integral orientation when the
first displayed double coset belongs to the paired-axis family, and sends
it to zero otherwise.  Formula (3.180) is not a zero vector in general and
does not disprove global anisotropy.  It proves that neither the diagonal
square nor the single-axis energy is a leading coefficient for arbitrary
multi-axis support.  The exact next group-theoretic obligation is to
analyze finite signed cycles in the cross-incidence graph generated by
\(\Phi_I\) together with the reverse bucket in (3.180).

### 3.28 The direct collider has no finite orbit

The directed map \(\Phi_I\) itself is radially escaping.  First suppose
that \(g_I\) is hyperbolic in the Bass--Serre tree
\(\mathcal T_{\rm BS}\) of \(Q=H*\langle t\rangle\).  Let
\(\mathcal A_I\) be its translation axis, let \(\tau_I>0\) be its
translation length, and put

\[
 r_I(x):=d_{\mathcal T_{\rm BS}}(x,\mathcal A_I).
 \tag{3.181}
\]

The conjugate \(r_{g_I^{-1}x}=g_I^{-1}r_xg_I\) is the nontrivial
stabilizer of the \(H\)-vertex \(z=g_I^{-1}x\), so
...
If \(r_I(x)>0\), the projections of \(z\) and \(x\) onto the axis differ
by one \(g_I\)-translation.  Hence the geodesic \([z,x]\) passes through
the axis and has length

\[
 d(z,x)=\tau_I+2r_I(x).
\]

An \(H\)-vertex has degree two in \(\mathcal T_{\rm BS}\), and its
nontrivial stabilizer swaps its two incident edges.  The image
\([z,r_zx]\) therefore leaves \(z\) through the component away from the
axis.  It cannot return to the axis in a tree, so

\[
 \boxed{r_I(\Phi_I(x))=\tau_I+3r_I(x)
 \qquad(r_I(x)>0).}
 \tag{3.183}
\]

If \(r_I(x)=0\), both \(z\) and \(x\) lie on the axis and the reflected
segment initially follows the opposite axis ray.  Its endpoint cannot
remain on the axis: the only point there at distance \(\tau_I\) in that
direction is \(g_I^{-2}x\), which would make (3.177) identify
\(\mathscr D_I(x)\) with its inverse, contrary to (3.174).  Thus

\[
 \boxed{0<r_I(\Phi_I(x))<\tau_I
 \qquad(r_I(x)=0).}
 \tag{3.184}
\]

The strict upper bound records the nonempty initial overlap with the
opposite axis ray; its exact length depends on the cyclic word.

For \(g_I=t^{-1}\), let \(\mathcal A_I\) denote the
\(\langle t\rangle\)-vertex fixed by \(g_I\).  Every \(H\)-vertex has
positive distance from it.  The two vertices \(x,g_I^{-1}x\) lie in
distinct branches at the fixed vertex, and the same reflection argument
gives

\[
 \boxed{r_I(\Phi_I(x))=3r_I(x).}
 \tag{3.185}
\]

Equations (3.183)--(3.185) prove that \(\Phi_I\) has no finite orbit.  More
strongly, for every nonempty finite vertex set \(S\), some
\(x\in S\) has \(\Phi_I(x)\notin S\): choose maximal positive radius, or
use (3.184) if all hyperbolic radii are zero.  Therefore the first two
rows of (3.179) cannot telescope around a finite cycle using only direct
\(\Phi_I\)-edges.  Any complete cancellation must use the reverse bucket
...
### 3.98a Frozen ledger and convergence budget

The remote ledger is frozen at the six terms displayed in (3.646): the
evaluated baseline $n_s$ and the five unevaluated terms
$\overline\kappa$, $\mathcal S$, $\mathcal T$, $\mathcal C$, and
$\mathcal O$.  A refinement may split one of those terms internally for
...
1. computes one of the five unevaluated terms;
2. proves that term vanishes;
3. absorbs it into another existing term without creating a new residual;
   or
4. reduces it to an explicit finite theorem whose inputs and parity output
   are both stated, rather than merely naming another family of cases.
...
Choose $U_E$ so that the terminal-localization conclusion (3.552) holds
whenever $|u|_K>U_E$.  For the rest of Sections 3.100--3.101, fix a target
$u$ satisfying
...
Retain occurrence tags on the actual active
...
Retain (3.658a) and the extremal paired-bucket hypothesis of Section 3.77.
Thus terminal localization (3.552) and remote multiplier rigidity (3.558)
are both in force throughout this section.
...
### 3.106 The duplicate-prefix contraction is three coefficient paths

Fix a source coordinate $v$ and abbreviate the transported occurrence
label $x_{o,v}$ from (3.347) to $x_o$.  Reduce all five coordinate
coefficients modulo two.  The natural paired-edge chain at $v$ is

\[
 \begin{aligned}
 \mathsf C_v={}&
 \overline{F_0(v)}
 \left([x_3,x_4]+[x_7,x_8]+[x_{11},x_{12}]\right)\\
 &+\overline{F_1(v)}
 \left([x_2,x_5]+[x_{10},x_{13}]\right)\\
 &+\overline{F_2(v)}[x_1,x_6]
 +\overline{F_3(v)}[x_9,x_{14}]
 +\overline{F_4(v)}[x_{15},x_{16}].
 \end{aligned}
 \tag{3.682}
\]

Here $[x,y]$ is an abstract mod-two edge whose endpoints retain their
occurrence provenance and carry the displayed transported labels.  The
exact prefix identities (3.674) give
...
After contracting precisely these equal vertices, (3.682) is the disjoint
union of the three coefficient paths
...
\[
 ...
 \mathsf P_{\rm L}(v):&
 &x_4
 &\overset{\overline{F_0(v)}}{\longleftrightarrow}
 [x_3=x_2]
 \overset{\overline{F_1(v)}}{\longleftrightarrow}
 x_5,\\
 \mathsf P_{\rm M}(v):&
 &x_8
 &\overset{\overline{F_0(v)}}{\longleftrightarrow}
 [x_7=x_6]
 \overset{\overline{F_2(v)}}{\longleftrightarrow}
 [x_1=x_{16}]\\
 &&&\overset{\overline{F_4(v)}}{\longleftrightarrow}
 [x_{15}=x_{14}]
 \overset{\overline{F_3(v)}}{\longleftrightarrow}
 x_9,\\
 \mathsf P_{\rm R}(v):&
 &x_{12}
 &\overset{\overline{F_0(v)}}{\longleftrightarrow}
...
Equivalently, the local degree chain is the following literal row:

\[
 \boxed{
 \begin{array}{c|c}
 \text{contracted label vertex}&[\delta_x](\partial\mathsf C_v)\\ \hline
 x_4&\overline{F_0(v)}\\
 [x_3=x_2]&\overline{F_0(v)}+\overline{F_1(v)}\\
 x_5&\overline{F_1(v)}\\
 x_8&\overline{F_0(v)}\\
 [x_7=x_6]&\overline{F_0(v)}+\overline{F_2(v)}\\
 [x_1=x_{16}]&\overline{F_2(v)}+\overline{F_4(v)}\\
 [x_{15}=x_{14}]&\overline{F_4(v)}+\overline{F_3(v)}\\
 x_9&\overline{F_3(v)}\\
 x_{12}&\overline{F_0(v)}\\
 [x_{11}=x_{10}]&\overline{F_0(v)}+\overline{F_1(v)}\\
 x_{13}&\overline{F_1(v)}.
 \end{array}
 }
 \tag{3.685}
\]

Summing (3.685) over all source coordinates and then aggregating equal
transported labels gives

\[
 \boxed{
 \sum_v\partial\mathsf C_v=0.
 }
 \tag{3.686}
\]

Indeed, before the five contractions the left side is exactly the
mod-two reduction of $\sum_o\epsilon_oq_oF_{s_o}$, which vanishes by
(3.346); occurrence signs disappear modulo two.  Thus (3.684)--(3.685)
exhaust every forced same-coordinate prefix transfer among the eight
natural paired edges.  A nonzero local degree left in (3.685) can disappear
in (3.686) through an additional nonduplicate equality at the same source
