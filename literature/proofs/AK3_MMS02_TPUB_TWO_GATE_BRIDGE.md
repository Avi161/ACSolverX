...
Section 6.34 isolates the exact tag-coupled boundary without another
coordinate census.  The lifted quotient path acts affinely on the relation
module.  Its homogeneous Jacobian is tame and invertible, but this alone does
not control the translation curvature.  The three endpoint residuals generate
the module exactly when that one curvature vector vanishes in their
...
Section 6.39 uses that specific coefficient structure without expanding it.
Because the first endpoint row is $[A]$, both module generation and the
cancellation kernel reduce exactly to the two second coordinates
$\beta,\delta$.  The coefficient $\beta$ cannot be a unit, while a proof that
$\delta$ is a unit would give an explicit free generator of $Q$ and close
both relation-module gates.  A complete rank-two Whitehead check also proves
that the actual base group is not its cyclic shadow.  The unit status of
...
Section 6.75 closes every simple-target obstruction to the canonical tagged
gate, without assuming that the earlier \(A_5\) representation descends to
that gate.  In any quotient of \(Q_A*\langle t\rangle\), the normal closures
of the images of \(q\), \(B\), and \(Q_A\) coincide.  If that normal closure
is the whole target, both tagged pairs reduce to an identity-row pair and
Lemma 6.1 connects them.  Hence a separating quotient must have a proper
nontrivial normal image of \(Q_A\) and cyclic quotient generated by the tag;
no simple target can work.

Section 6.76 excludes every finite or soluble target as well.  Both tagged
pairs are normally generating and have identical images in the target
abelianization.  The finitary Andrews--Curtis theorem and Guyot's
soluble-group \(GACC1\) theorem therefore connect them in every finite or
finitely generated soluble quotient.  Combined with Section 6.75, any
separating quotient must be infinite, nonsoluble, and nonsimple, with a
proper nontrivial normal image of \(Q_A\) and cyclic quotient generated by
the tag.

Section 6.77 audits the published stable-AK(3) claim against both primary
texts.  The five automated sequences and the independently replayed
fifty-three-move corridor prove only \(P_{25}\sim_{\rm AC}\operatorname{AK}(3)\).
Their asserted first leg uses MMS02 stable triviality for a presentation
derived from the misprinted row.  Shehper et al.'s own corrected Appendix F
states that this presentation is not necessarily stably AC-trivial.  Thus
...
Section 6.79 audits a tempting generalized-AC shortcut for the tagged gate.
The Magnus base embedded in \(Q_A\) makes \(Q_A\) noncyclic and hence
rank two.  After any number of ordinary tag stabilizations, Grushko
additivity leaves the normally generating tuple one entry shorter than the
rank of its ambient free product.  It can therefore never be converted to a
generating tuple at that rank.  In particular, the positive theorem for
...
Section 6.82 evaluates the other explicit loop generators from (138) at the
exact level.  They and all vertical component conjugations preserve the
conjugacy class of each labelled row.  The canonical lifted first row
\(EB=D^{-1}qD\) is conjugate to \(q\), whereas Theorem 6.11 proves that
\(q\) is not conjugate to \(B\).  Hence no word in the full subgroup
generated by those loops can close the defect.  A successful lift must use a
move which changes a component conjugacy class.

Section 6.83 adds inversion and row permutation to that subgroup boundary.
The resulting multiplication-free action preserves the unordered pair of
unoriented component-conjugacy classes.  Free-product factor type and the
...
The separate [opposite-donor theorem](AK3_MMS02_PRIMITIVE_COMPLETION_TRACE_CHECK.md#opposite-donor-primitive-completion-is-excluded-literally)
excludes primitive completion here for every conjugator and integer slope:
the exact free-base equation adds a $b$-letter while its monodromy cannot
decrease their number. The preceding perfect-kernel control explains why
solvable images cannot prove this result. Both fixed-donor primitive
criteria are now closed, without excluding paths that change both rows.
...
### 6.34. The tag-coupled gate is affine curvature

The nonunit determinants in Theorem 6.26 do not decide whether all three
residual rows generate the relation module.  They also cannot be bypassed by
using only the homogeneous Jacobian of the quotient path.

Work in

\[
 M=F/[N,N],\hspace{1em}\mathcal R=N/[N,N],
 \tag{223}
\]

and use the fixed branch-B quotient path underlying (44).  Its quotient
source and target, together with their chosen lifts to $M^3$, are

\[
 \sigma_u=(1,1,\overline u),\hspace{1em}
 \sigma_v=(1,1,\overline v),\hspace{1em}
 \widetilde\sigma_u=(1,1,u),\hspace{1em}
 \widetilde\sigma_v=(1,1,v).
 \tag{224}
\]

Let $P$ denote that exact AC word and put
$\tau_P=P(\widetilde\sigma_u)$.  Its quotient is $\sigma_v$.  With the
left-residual convention, define

\[
 \kappa_{P,i}:=\tau_{P,i}\widetilde\sigma_{v,i}^{-1}
 \in\mathcal R\hspace{1em}(i=1,2,3).
\]

Relative to the same lifted references, the actual source and its endpoint
under $P$ have residual columns
...
\[
 ...
 \hspace{1em}
 e=
 \begin{pmatrix}[A]\\[E_B]\\[H_Bv^{-1}]\end{pmatrix}
 \in\mathcal R^3.
 \tag{225}
\]

Linearize each AC1--AC3 move along the displayed quotient path.  Multiplication
gives an elementary row update with quotient-prefix coefficients, inversion
gives a diagonal trivial unit, and conjugation gives a diagonal group unit.
Their product is therefore an AC-tame matrix, generated by elementary
matrices and diagonal trivial units $\pm g$:

\[
 J_P\in\operatorname{GL}_3(\mathbb ZG_-).
 \tag{226}
\]

Applying the same residual recurrences to the reference lift and the actual
source gives the exact affine identity

\[
 \boxed{e=J_Pb+\kappa_P\hspace{1em}\text{in }\mathcal R^3.}
 \tag{227}
\]
...
\[
 L_e=\mathbb ZG_-[A]+\mathbb ZG_-[E_B]
     +\mathbb ZG_-[H_Bv^{-1}]\leq\mathcal R,
 \hspace{1em}C_e=\mathcal R/L_e.
 \tag{228}
\]

Write a bar for passage to $C_e$.  Reducing (227) modulo $L_e$ gives

\[
 0=J_P\overline b+\overline{\kappa}_P
 \hspace{1em}\text{in }C_e^3.
 \tag{229}
\]

Since $J_P$ induces an automorphism of $C_e^3$, (229) has the exact
consequence
...
\[
 ...
 \overline{\kappa}_P=0\text{ in }C_e^3.}
 \tag{230}
\]

Indeed, the forward implication is immediate because $C_e=0$.  Conversely,
if $\overline{\kappa}_P=0$, invertibility of $J_P$ gives
$\overline b=0$.  The first two components of $b$ are the basis $[A],[B]$ of
$\mathcal R$ from (216), so $C_e=0$.

**Theorem 6.27 (affine-curvature boundary).**  Homogeneous invertibility of
the lifted AC path does not by itself prove that the tag-coupled endpoint
generates the relation module.  For the fixed canonical branch-B lift, that
generation statement is equivalent exactly to (230).

Thus the next obligation is one relative filling: express every component of
$\kappa_P$ by the three endpoint residuals in (228), or prove that one
component survives in $C_e$.  A positive module filling would still give only
a stable normalization of the abstract residual system
$([A],[E_B],[H_Bv^{-1}])$ after the required stable completion is supplied.
It would not normalize the tagged endpoint in $M$, give a free crossed-module
lift, or supply a donor-legal AC path.  A negative filling result would
obstruct only this fixed quotient-path lift.  No alternative-path curvature
...
### 6.39. The specific kernel is one two-coordinate syzygy

The fixed first endpoint residual is already the first basis vector.  Use
the unique coordinates supplied by (216) to write

\[
 \begin{aligned}
 [E_B]&=\alpha[A]+\beta[B],\\
 [H_Bv^{-1}]&=\gamma[A]+\delta[B]
 \end{aligned}
 \hspace{1em}(\alpha,\beta,\gamma,\delta\in S).
 \tag{250}
\]

Thus the rectangular matrix in (234) has the exact form
...
Define the left-linear row map

\[
 \lambda_{\beta,\delta}:{}_SS^2\longrightarrow{}_SS,
 \hspace{1em}(r,s)\longmapsto r\beta+s\delta,
 \hspace{1em}P_{\beta,\delta}:=\ker\lambda_{\beta,\delta}.
 \tag{252}
\]

The first row of (251) absorbs every first coordinate.  Consequently
...
\[
 ...
 S\beta+S\delta=S
 \Longleftrightarrow
 \lambda_{\beta,\delta}\text{ is onto}.}
 \tag{253}
\]

There is also an exact identification of the cancellation kernel, without
assuming (253):

\[
 \begin{aligned}
 \Theta:P_{\beta,\delta}&\longrightarrow Q,\\
 (r,s)&\longmapsto(-r\alpha-s\gamma,r,s).
 \end{aligned}
 \tag{254}
\]

Indeed, multiplying the displayed row in (254) by (251) gives
$(0,r\beta+s\delta)$, while projection of a row in $Q$ onto its final two
coordinates is the inverse of $\Theta$.  Hence

\[
 \boxed{Q\cong P_{\beta,\delta}.}
 \tag{255}
\]

This makes one sharp sufficient target visible.  If $\delta$ is a unit, then
(253) holds and $Q$ is freely generated by

\[
 \left(-\alpha+\beta\delta^{-1}\gamma,
 1,-\beta\delta^{-1}\right).
 \tag{256}
\]

The symmetric formula when $\beta$ is a unit is

\[
 \left(\delta\beta^{-1}\alpha-\gamma,
 -\delta\beta^{-1},1\right).
 \tag{257}
\]

Theorem 6.26 already rules out the second shortcut:
$\chi_*(\beta)=p_B(t)$ is not a Laurent unit, so $\beta$ cannot be a unit of
$S$.  By contrast, Theorem 6.28 says only
$\chi_*(\delta)=t$.  The cyclic base change does not reflect units, so this
does not prove that $\delta$ is a unit.

Nor can the coefficient problem be collapsed by identifying the actual base
group with its cyclic shadow.  Rename $y,z$ as $x,y$ in the relator $R_*$ of
(215).  The Whitehead automorphism $x\mapsto xy$, $y\mapsto y$ reduces its
cyclic length from $26$ to the canonical word

```text
XXXYYXyxxxyXYYXXXYxyyxY
```

of length $23$.  The complete rank-two Whitehead table has no
length-decreasing move from this word.  Whitehead's primitive-word theorem
therefore shows that $R_*$ is not primitive.  If $G_-$ were cyclic, (217)
would force $G_-\cong\mathbb Z$.  The kernel of the resulting epimorphism
$F(y,z)\to\mathbb Z$ is normally generated by a primitive basis letter.
Magnus's normal-closure theorem would then make $R_*$ conjugate to that
letter or its inverse, a contradiction.  Thus
...
**Theorem 6.32 (specific syzygy reduction).**  Conditional on the curvature
filling (230), the legal relation-module cancellation gate is exactly the
freeness of the rank-one syzygy $P_{\beta,\delta}$ in (252).  An exact proof
that $\delta\in S^\times$ would close both module generation and this
cancellation gate, with the explicit generator (256).  The other
single-coordinate unit shortcut and the cyclic-base collapse are impossible.
...
### 6.75. Simple targets cannot separate the tagged gate

Return to the canonical tagged gate (130).  Put

\[
 \mathcal G_t:=Q_A*\langle t\rangle
\]

and let

\[
 \pi:\mathcal G_t\twoheadrightarrow K
\tag{472}
\]

be any quotient.  Write
...
\[
 N:=\operatorname{Ncl}_K(\pi(Q_A)).
\tag{473}
\]

Theorem 6.12 says that \(q\) and \(B\) each normally generate \(Q_A\).
Taking their normal-closure factorizations through \(\pi\), and then allowing
conjugation by all of \(K\), gives the exact equality

\[
 \boxed{
 N
 =\operatorname{Ncl}_K(\bar q)
 =\operatorname{Ncl}_K(\bar B).
 }
\tag{474}
\]

Suppose first that \(N=K\).  Then both \(\bar q\) and \(\bar B\)
individually normally generate \(K\).  Choose a finite factorization of
\(\tau^{-1}\) by conjugates of \(\bar q^{\pm1}\).  The usual donor macro
multiplies those factors into the second row while restoring the first and
therefore gives

\[
 (\bar q,\tau)\sim_{\rm AC}(\bar q,1).
\tag{475}
\]

The same construction is reversible for \(\bar B\).  After swapping the
two rows, Lemma 6.1 applies to the two normal generators and yields
...
If \(N=1\), then (474) gives \(\bar q=\bar B=1\), so the two image pairs
are already identical.  Consequently a quotient can separate the tagged
pairs only in the remaining case
...
Because \(K\) is generated by \(\pi(Q_A)\) and \(\tau\), equations
(473) and (477) also force

\[
 \boxed{
 K/N=\langle\tau N\rangle,
 }
\tag{478}
\]

so the quotient above the proper normal image is cyclic.

**Theorem 6.69 (tagged quotient boundary).**  No quotient (472) in which
the image of \(Q_A\) normally generates \(K\) can obstruct (130).  In
particular, no nontrivial simple target can separate the two tagged pairs:
if \(\pi(Q_A)=1\), their images coincide, while otherwise simplicity gives
\(N=K\) and (476).

The previously used \(A_5\) representation of
\(G_-=F/\operatorname{Ncl}(A,B)\) does not descend to
\(Q_A=F/\operatorname{Ncl}(A,v)\): the pinned evaluation (393) is
\(\rho(v)=(2,0,4,1,3)\ne1\).  Thus it supplies no tagged-gate instance.
The theorem instead excludes every genuine simple-target image of the
tagged gate at once.  A finite quotient obstruction, if one exists, must
retain a proper nontrivial normal subgroup \(N\) as in (477), together with
the cyclic tag extension (478).  No such extension is constructed or
classified here.  The relative Peiffer class, unrestricted MMS02 bridge,
stable AK(3), ordinary AK(3), stable Andrews--Curtis, and Andrews--Curtis
remain open.

### 6.76. Finite and soluble targets cannot separate the tagged gate

Let (472) now have a target \(K\) which is finite or soluble.  The pairs
\((q,t)\) and \((B,t)\) normally generate \(\mathcal G_t\): killing \(q\),
respectively \(B\), kills \(Q_A\) by Theorem 6.12, and killing \(t\)
removes the free tag factor.  Their image pairs

\[
 (\bar q,\tau),\qquad(\bar B,\tau)
\tag{479}
\]

therefore normally generate \(K\).

Their abelianized images are not merely equivalent; they are equal.  The
literal identity \(B=qD^{-1}\) from (2), with
\(D={}^h[X,r]\), gives

\[
 \bar B\equiv\bar q\pmod{[K,K]},
\tag{480}
\]

while the tag coordinate is unchanged.  Hence
...
Two primary connectivity theorems apply to this exact situation.

1. Theorem 1.1 of Borovik--Lubotzky--Myasnikov,
   [“The Finitary Andrews--Curtis Conjecture”](https://arxiv.org/abs/1103.1295),
   says that for every finite group \(G\) and
   \(n\ge\max\{w(G),2\}\), two normally generating \(n\)-vectors are
   AC-equivalent exactly when their abelianized vectors are AC-equivalent.
2. Theorem 1.3 of Luc Guyot,
   [“On Andrews--Curtis conjectures for soluble groups”](https://arxiv.org/abs/1612.06912),
   proves the same statement for every finitely generated soluble group.

The quotient \(K\) is finitely generated, and (479) gives \(w(K)\le2\).
Thus \(n=2\) is admissible in either theorem, and the equality (481) proves

\[
 \boxed{
 (\bar q,\tau)\sim_{\rm AC}(\bar B,\tau)
 \quad\text{in every finite or soluble quotient }K.
 }
\tag{482}
\]

**Theorem 6.70 (finite/soluble tagged-quotient boundary).**  No finite or
finitely generated soluble quotient of \(Q_A*\langle t\rangle\) separates
the canonical tagged pairs.  Together with Theorem 6.69, any separating
quotient must be infinite, nonsoluble, and nonsimple and must fit the proper
cyclic-extension shape (477)--(478).

This is an image-orbit theorem, not a lifting theorem.  It does not lift a
target AC path through the kernel of (472), evaluate the relative Peiffer
class, or produce a free-group path for (130).  It constructs no nonsoluble
extension and proves no MMS02 bridge, stable AK(3), ordinary AK(3), stable
Andrews--Curtis, or Andrews--Curtis claim.
...
That paper supplies five AC sequences from \(P\) to AK(3).  Its first and
shortest published predecessor is the fifty-three-move Appendix F sequence
(281), already replayed independently in Theorem 6.37.  Thus the second leg

\[
 \boxed{
 P_{25}\sim_{\rm AC}\operatorname{AK}(3)
 }
\tag{483}
\]
...
The stable conclusion in both papers needs a separate first leg.  Lisitsa
states that \(P\) belongs to a family proved stably AC-trivial in MMS02 and
then composes that assertion with (483).  The source of \(P_{25}\), however,
is not the corrected Wirtinger presentation \(W\).  It is the presentation
\(W'\) obtained from the printed MMS02 row

\[
 x_{13}=x_5x_{12}x_5^{-1}
\tag{484}
\]

in place of the corrected row

\[
 x_{13}=x_4x_{12}x_4^{-1}.
\tag{485}
\]

Appendix F of Shehper et al.,
[“What makes math problems hard for reinforcement learning”](https://arxiv.org/abs/2408.15332),
checks the consequence explicitly: \(W'\) is not a Wirtinger presentation of
any knot diagram, and deleting different rows gives nonisomorphic groups.
The correct unknot theorem, Proposition 12/Theorem 16 of that paper, applies
to \(W\), not \(W'\).

Most decisively, after displaying the same \(P_{25}\) and the path (483),
the corrected Appendix F says that, unlike a presentation obtained from a
correct Wirtinger presentation, these presentations are not necessarily
stably AC-trivial.  Therefore the implication used by the published
composition is precisely

\[
 \boxed{
 P_{25}\stackrel{?}{\sim}_{\rm stAC}(x,y),
 }
\tag{486}
\]

which neither Appendix F nor the automated AC sequences prove.  The latter
never use a stable move; they certify only (483).

**Theorem 6.71 (published-claim scope audit).**  The displayed proof chain
in the two cited papers does not establish stable AC-triviality of AK(3).
Its verified content is the ordinary AC equivalence (483); its missing
premise is the stable triviality (486) of the misprint-derived
representative.  Consequently (483) transports a future proof of (486) to
AK(3), but supplies no such proof itself.

This is a scope diagnosis, not a negative invariant.  It does not show that
\(P_{25}\) or AK(3) is stably nontrivial, invalidate the certified AC path,
or decide another stable representative.  The direct MMS02 bridge, tagged
...
[“Andrews--Curtis and Nielsen equivalence relations on some infinite
groups”](https://arxiv.org/abs/1304.2668), proves abelianization-controlled
AC equivalence for finitely generated groups in the class \(\mathfrak C\) in
which every maximal subgroup is normal.  Proposition 2.1 there identifies
the operative property: every normally generating tuple is already a group-
generating tuple.

The tagged ambient group has the opposite property.  Section 4 embeds the
free Magnus base \(F(a,b)\) in \(Q_A\), so \(Q_A\) is noncyclic.  Since its
presentation (8) has two generators,

\[
 \operatorname{rank}(Q_A)=2.
\tag{494}
\]
...
\[
 G_k=Q_A*F(t,s_1,\ldots,s_k),\qquad
 \mathbf u_k=(q,t,s_1,\ldots,s_k).
\tag{495}
\]

Theorem 6.12 says that \(q\) normally generates \(Q_A\), so
\(\mathbf u_k\) normally generates \(G_k\).  Grushko rank additivity gives

\[
 \operatorname{rank}(G_k)=2+(k+1)=k+3,
 \qquad |\mathbf u_k|=k+2.
\tag{496}
\]

Every AC transformation preserves tuple length.  Consequently no tuple in
the AC orbit of \(\mathbf u_k\) can generate \(G_k\) as a group.  This
remains true after every finite number of ordinary stabilizations: each new
tag increases both quantities in (496) by one and preserves the deficit.
Already at \(k=0\), the failure can also be seen from the retraction
\(G_0\to Q_A\) which kills \(t\): the subgroup generated by \((q,t)\) maps
to the cyclic subgroup \(\langle q\rangle\ne Q_A\).

**Theorem 6.73 (tagged generating-rank boundary).**  Neither the
class-\(\mathfrak C\) theorem nor a strategy which first converts the
tagged tuple into a generating tuple can prove (130), at any finite tag-
stabilized rank.  The tuple is normally generating but is always one entry
short of the ambient group rank.

This theorem does not show that two normally generating tuples of the same
length lie in different AC orbits.  AC equivalence is defined precisely on
such tuples and need not pass through a group-generating tuple.  Thus no
relative Peiffer invariant, literal path, MMS02 bridge, stable AK(3),
ordinary AK(3), stable Andrews--Curtis, or Andrews--Curtis conclusion
...
### 6.82. Pure component-conjugation loops preserve the wrong class

Let \(\mathscr C\) be the subgroup of full tuple transformations generated
by arbitrary component conjugations

\[
 C_1(g)(a,b)=({}^ga,b),\qquad
 C_2(g)(a,b)=(a,{}^gb),
 \qquad g\in G_t,
\tag{503}
\]

together with the two tag-interchange loops in (138).  Each of those loops
already lies in the subgroup generated by (503): the first conjugates the
tag row by the current first row and then by the fixed ambient element
\(B^{-1}\), while the second conjugates the first row by the current tag row
and then by \(t^{-1}\).  Neither expansion contains a multiplication,
inversion, or swap.  The same is true of their inverses.

It follows that every transformation in \(\mathscr C\) preserves the
conjugacy class of each labelled component.  This statement does not require
the conjugator in (503) to be fixed in advance: a word built from the current
rows is still one element of \(G_t\) at the moment the component is
conjugated.

For the canonical lift,
...
so its first row is conjugate to \(q\).  If a word in \(\mathscr C\)
sent \((EB,t)\) to \((B,t)\), then \(q\) and \(B\) would be conjugate in
\(G_t=Q_A*\langle t\rangle\).  Both are nontrivial elements of the
\(Q_A\) free factor, so free-product conjugacy reduces this to conjugacy in
\(Q_A\), contrary to Theorem 6.11.

**Theorem 6.76 (pure-conjugation lifted-loop boundary).**  No composition of
vertical component conjugations and the two tag-interchange loops (138)
closes the canonical exact defect.  Equivalently, the complete subgroup
\(\mathscr C\) preserves the wrong first-component conjugacy class.

This is an infinite-subgroup theorem, not a bounded loop enumeration.  It
does not address a path containing row multiplication, inversion, swap, or
another move which changes a component conjugacy class.  Combined with
Theorem 6.75, it shows that a successful lifted loop must go beyond both the
pure-conjugation subgroup and the single vertical-multiplication commutator
\(P C_1(k)P^{-1}\).  The exact Peiffer gate, MMS02 bridge, stable AK(3),
ordinary AK(3), stable Andrews--Curtis, and Andrews--Curtis remain open.
No further pure-conjugation loop family is opened.

### 6.83. Every tagged path has at least two row multiplications

For a pair \((a,b)\) in a group, let

\[
 \mathfrak U(a,b)=
 \bigl\{\{[a],[a^{-1}]\},\{[b],[b^{-1}]\}\bigr\},
\tag{505}
\]

where brackets denote conjugacy classes and the outer braces are unordered.
Component conjugation, row inversion, and row permutation preserve
\(\mathfrak U\).  These are exactly the elementary AC operations which do
not multiply one row by the other.

For the canonical lifted source, (504) gives

\[
 \mathfrak U(EB,t)=
 \bigl\{\{[q],[q^{-1}]\},\{[t],[t^{-1}]\}\bigr\}.
\tag{506}
\]

Nontrivial elements of distinct free factors of
\(G_t=Q_A*\langle t\rangle\) are not conjugate.  Thus a hypothetical equality
\(\mathfrak U(EB,t)=\mathfrak U(B,t)\) must match the tag classes to the tag
classes and would force \(q\) to be conjugate to either \(B\) or \(B^{-1}\)
in \(Q_A\).  The first possibility is excluded by Theorem 6.11.  For the
second, use the abelianization from (87).  Since \(q=Xy\), while
\(D\in Q_A'\) and \(B=qD^{-1}\),

\[
 \chi(q)=\chi(B)=-1,\qquad \chi(B^{-1})=1
 \quad\text{in }Q_A^{\rm ab}\cong\mathbb Z.
\tag{507}
\]

Conjugate elements have the same abelianization, so that possibility is
also excluded.  Hence no multiplication-free path closes the tagged gate.

There is a second, independent homological consequence.  In the ordered
basis of

\[
 G_t^{\rm ab}\cong Q_A^{\rm ab}\oplus\langle t\rangle
 \cong\mathbb Z^2,
\]

the source and target row matrices are both
...
Conjugations act trivially on this matrix, inversions and permutations act
by signed permutation matrices, and one row multiplication acts by one
elementary transvection.  A signed permutation times an elementary
transvection times a signed permutation cannot be the identity: one of its
rows still has two nonzero entries.  Therefore a path with exactly one row
multiplication is impossible as well.

**Theorem 6.77 (tagged multiplication-depth boundary).**  Every AC path
from \((q,t)\), equivalently from its conjugate representative \((EB,t)\),
to \((B,t)\) in \(Q_A*\langle t\rangle\) contains at least two row
multiplications.

This is a global depth lower bound with arbitrary conjugations, inversions,
...
24. the actual base presentation is aspherical with free rank-two relation
    ...
    quotient transfer;
25. the homogeneous Jacobian of the fixed branch-B quotient lift is tame and
...
65. every quotient image of the tagged gate in which \(Q_A\) normally
    generates the target is AC-connected by the redundant-row transfer;
    hence every simple target is vacuous, while proper cyclic extensions of
    the normal image remain admissible; and
66. the finitary Andrews--Curtis theorem and Guyot's soluble-group
    \(GACC1\) theorem connect the two tagged image pairs in every finite or
    soluble target because their abelianized vectors are identical; these
    quotient paths need not lift through the kernel; and
67. the published automated sequences certify
...
68. the raw fixed \(g=V,j=U^{-1}\) family has a class-two endpoint solution
    ...
    and
69. after every finite tag stabilization the tagged tuple is one entry
...
72. the subgroup generated by vertical component conjugations and the two
    tag-interchange loops (138) preserves both labelled component-conjugacy
    classes, so it cannot change the canonical class of \(q\) into that of
    \(B\); and
...
Theorem 6.69 returns to the higher-value tagged gate rather than extending
the signed-skeleton quotient census.  In every quotient of
\(Q_A*\langle t\rangle\), the images of \(q\), \(B\), and \(Q_A\) have one
common normal closure.  If it is the whole target, the tag can be cleared
and Lemma 6.1 connects the two pairs; if it is trivial, the pairs already
coincide.  Thus every genuine simple-target test is vacuous.  A separating
quotient must instead be a proper cyclic extension of the nontrivial normal
image of \(Q_A\).  This is only a quotient-shape theorem: it constructs no
such extension, no tagged Peiffer path, and no MMS02 or AK(3) conclusion.
Theorem 6.70 uses the exact abelian coincidence \(B=qD^{-1}\), with \(D\)
a commutator, together with the finitary Andrews--Curtis theorem and
Guyot's soluble-group \(GACC1\) theorem.  The two image pairs are therefore
AC-equivalent in every finite or finitely generated soluble quotient.
Combined with Theorem 6.69, a separating target must be infinite,
nonsoluble, nonsimple, and a proper cyclic extension of the normal image of
\(Q_A\).  This still gives only the shape of a possible quotient
obstruction, not a Peiffer lift, bridge, or AK(3) theorem.
Theorem 6.71 reconciles the branch with the published stable-AK(3) claim.
The Lisitsa sequences and the independently replayed Appendix F corridor
prove the ordinary equivalence \(P_{25}\sim_{\rm AC}\operatorname{AK}(3)\).
The asserted stable first leg comes from the misprinted MMS02 row, whereas
the corrected Shehper Appendix F says that the resulting presentations are
...
Theorem 6.73 excludes the generating-tuple shortcut at every finite tag
rank.  The tagged tuple in
\(Q_A*F(t,s_1,\ldots,s_k)\) has \(k+2\) entries while the ambient group has
...
Theorem 6.76 closes the other explicit loop subgroup from (138).  Every
generator there only conjugates one labelled component, so every word in the
subgroup preserves the first-row conjugacy class of
\(EB=D^{-1}qD\).  Reaching \(B\) would conjugate \(q\) to \(B\), contrary
to Theorem 6.11.  A successful path must therefore use a class-changing
Nielsen move outside this pure-conjugation subgroup.
Theorem 6.77 strengthens this to the full multiplication-free action.
Its invariant is the unordered pair of unoriented component-conjugacy
classes; free-product factor type, literal nonconjugacy, and the nonzero
