...
`d8ff2cfe1976d3c1fe7657c18a6b66b0e6fcb1083450cb5adb0d5682b0b025f3`.
...
Its verification is one constant-size exact replay, not a search or census,
and it makes no noncancellation, lift, AC, or stable-AC claim.
...
## Verified rows zero and one, columns zero through three
...
| Entry | signed reduced monomials \(a\,m\), each annotated \((\nu_t(m),c(m))\) |
|---|---|
...
The guarded checker independently replays this derivation route and compares
the fully reduced entries with the displayed monomial dictionary.

## Verified c-count-zero monomials across the full \(\mathbf H^\circ\) matrix

These are the only c-count-zero monomials in the complete matrix; row four
vanishes.  The checker compares this extracted pure-\(t\) dictionary
separately.  They are candidates for depth preservation under the coarse
bound only.
...
in a Hessian contribution.  The verified rows-zero-and-one expansions are
not printed in the cited proof formulas, and neither the height nor this
left-multiplier bound controls collisions, the Green-flow support, the
target bucket, or the affine unary term.  In particular, the table does not
establish a leading-term or maximal-ray separation statement.
