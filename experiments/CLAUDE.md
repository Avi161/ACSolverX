...
- Only the parent writes the jsonl. `_emit`/`_write_path`/`_update_row` go through `_persist` (flush **and** fsync) because a Drive-mounted file dropped flushed rows; `_report_lost_rows` is the backstop. A guard-tripped presentation has no row until the pool finishes. [a hole is not a write race](lessons/jsonl-hole-is-not-a-write-race.md)
