...
| [`stable_ac/cov_top3_ms640_abel.ipynb`](stable_ac/cov_top3_ms640_abel.ipynb) + [`_len`](stable_ac/cov_top3_ms640_len.ipynb) | `experiments.stable_ac.cov.run.cov_top3_run` — the two CoV top-3 rules (abelian mass / shortest transformed pair) over all 640 ms640 presentations at budget 100,000, early-exiting at the first solve. **One notebook per parallel session, one rule each**; they differ only in `RULE`. Method + gates: [`COV_TOP3_MS640.md`](../stable_ac/cov/run/COV_TOP3_MS640.md) | `results/stable_ac/cov/cov_top3/` |
