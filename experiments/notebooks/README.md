...
| [`stable_ac/cov_top3_ms640_refined.ipynb`](stable_ac/cov_top3_ms640_refined.ipynb) | `experiments.stable_ac.cov.run.cov_top3_relabel` (stage A) + `cov_top3_run` under a scoped rule registration — the same 640 × 3 × 100,000 pipeline as the two above, over the **refined** rankings: five rules (`abel_rd`, `len_rd`, `abel_len_rd`, `abel_len_rd_mk`, `len_rd_mk`) that add a total-length term, an `MK` tie-break, and a filter dropping any pick that is an earlier pick with `x`/`y` renamed. `RULE` is the only knob that differs between sessions; the RUN cell preflights the manifest (re-deriving every relabel class, not trusting the stored tag) before spending a node. Subset-60 says the dedup is inert and `MK` changes sign with the budget — this run is the powered test. Evidence: [`COV_RELABEL_B1K.md`](../../results/comparison/COV_RELABEL_B1K.md) | `results/stable_ac/cov/cov_top3/` |
