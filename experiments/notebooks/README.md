...
| [`stable_ac/cov_top3_ms640_refined.ipynb`](stable_ac/cov_top3_ms640_refined.ipynb) | `experiments.stable_ac.cov.run.cov_top3_relabel` (stage A) + `cov_top3_run` under a scoped rule registration — the same 640 × 3 × 100,000 pipeline as the two above, over the **refined** rankings: seven rules that add a total-length term, a third ordering term, and a filter dropping any pick that is an earlier pick with `x`/`y` renamed. The rule is **abel → length → `S`** (`abel_len_rd_s`, `len_rd_s`); the `_mk` pair is kept only so `MK` stays comparable and is never the recommendation. `RULES` is a list and the RUN cell walks it in order, one arm fully finished before the next, preflighting every manifest in the list (re-deriving each relabel class, not trusting the stored tag) before spending a node. The budget-1,000 run over all 640 already showed `S` buys rank 1 (584 → 590) and costs ranks 2–3 (580/580 → 524/543), so this run is the powered test of cost at production budget. Evidence: [`COV_RELABEL_B1K.md`](../../results/comparison/COV_RELABEL_B1K.md) | `results/stable_ac/cov/cov_top3/` |
