...
@njit(cache=True)
def adjust_scores(blob, offsets, lengths, scores, w_weight=2.0):
    for i in range(len(scores)):
        o = offsets[i]
        scores[i] += w_weight * min(response(blob[o:o + lengths[i]]))


@njit(cache=True)
def score_key(codes, whitehead, w_weight=2.0, s_weight=20.0, mk_weight=2.0):
    sep = 0
    ...
    score = f[0] + mk_weight * f[5] + s_weight * f[7]
    if whitehead:
        score += w_weight * min(response(codes))
    return score
...
def run(pair, arm, budget=1000, cap=48, *, w_weight=None,
        s_weight=None, mk_weight=None):
    ...
    if w_weight is not None:
        if not math.isfinite(w_weight) or w_weight < 0:
            raise ValueError('W weight must be finite and nonnegative')
        if arm not in ('s20', 'whitehead2', 'aut_edges'):
            raise ValueError('W weight requires an S20-based mixed-search arm')
        w_weight = float(w_weight)
    for name, value in [('S', s_weight), ('MK', mk_weight)]:
        if value is not None:
            if not math.isfinite(value) or value < 0:
                raise ValueError(f'{name} weight must be finite and nonnegative')
            if arm not in ('s20', 'whitehead2', 'aut_edges'):
                raise ValueError('Structural weights require a mixed-search arm')
    s_weight = 20.0 if s_weight is None else float(s_weight)
    mk_weight = 2.0 if mk_weight is None else float(mk_weight)
    ...
    if arm in ('greedy', 's20', 'whitehead2', 'aut_edges'):
        return mixed_search(pair, arm, budget, cap, w_weight, s_weight, mk_weight)
    start = pair
    witness = None
    preprocessing = 0.0
    config = S20_MK2
