...
| `run/cov_top3_manifest.py` + `run/cov_top3_run.py` | the two CoV **top-3** rules over all 640 ms640 presentations — `abel` (abelianized magnitude) and `len` (shortest transformed pair), one Colab session each. Stage A ranks each CoV family and freezes the top 3 per rule (zero search), Stage B searches them in rank order at budget 100,000 and stops at the first solve (≤ 300,000 nodes per presentation). Every row carries the search, the pick, the running `cum_nodes` **and** the plain-greedy reference for the same presentation, so the nodes/path comparison needs no join. The rule is in the filename, in every manifest row and in every result row, and is asserted before the first search — the two arms cannot resume into each other. Method, gates and why the solve count saturates on this dataset: [`COV_TOP3_MS640.md`](cov/run/COV_TOP3_MS640.md); tests in [`tests/stable_ac/test_cov_top3.py`](../../tests/stable_ac/test_cov_top3.py). |
