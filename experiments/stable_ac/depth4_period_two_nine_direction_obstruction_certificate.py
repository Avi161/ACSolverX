...
INVERSE_FOUR_POINT_ACTION = (
    (0, 2, 1, 3),
    (3, 0, 1, 2),
)


INVERSE_FOUR_POINT_COVECTOR = (1, 1, 1, 1, 1, 1)
...
def obstruction_bits(value: escape.WedgeVector) -> tuple[int, ...]:
    previous = eight.obstruction_bits(value)
    vector = remote.finite_wedge_vector(value, *INVERSE_FOUR_POINT_ACTION)
    new_bit = sum(
        left * right
        for left, right in zip(INVERSE_FOUR_POINT_COVECTOR, vector)
    ) % 2
...
class PeriodTwoNineDirectionObstructionCertificate:
    forest_paths: tuple[tuple[str, str, str], ...]
    syzygy_entries: int
    syzygy_l1: int
    component_image_stats: tuple[tuple[int, int], ...]
    homogeneous_image: tuple[tuple[str, int], ...]
    eighth_and_ninth_direction_ranks_mod_two: tuple[int, int]
    ninth_residual_length: int
    ninth_kernel_length: int
    ninth_degree_two_terms: int
    ninth_degree_two_l1: int
    ninth_prior_mod_two_bits: tuple[int, ...]
    inverse_four_point_action: tuple[tuple[int, ...], tuple[int, ...]]
    inverse_four_point_covector: tuple[int, ...]
    inverse_four_point_defect: tuple[int, ...]
    inverse_four_point_value_mod_two: int
    inverse_four_point_operator_rank: int
    inverse_four_point_augmented_rank: int
...
def period_two_nine_direction_obstruction_certificate(
) -> PeriodTwoNineDirectionObstructionCertificate:
    ...
    eighth_direction = eight.reconstruct_from_forest_paths(operators)
    directions = (
    ...
    ninth_variables = escape.add_variables(base, syzygy)
    assert not lift.add_vectors(
        defect,
        escape.correction_image(ninth_variables, operators),
    )
    ninth_residual = escape.corrected_residual(ninth_variables)
    ninth_kernel = escape.schreier_word(ninth_residual)
    ninth_wedge = escape.degree_two(ninth_kernel)

    new_columns = remote.operator_columns(operators, *INVERSE_FOUR_POINT_ACTION)
    assert all(
        sum(
            left * right
            for left, right in zip(INVERSE_FOUR_POINT_COVECTOR, column)
        ) % 2 == 0
        for column in new_columns
    )
    new_defect = remote.finite_wedge_vector(ninth_wedge, *INVERSE_FOUR_POINT_ACTION)
    new_value = sum(
        left * right
        for left, right in zip(INVERSE_FOUR_POINT_COVECTOR, new_defect)
    ) % 2
    ...
    certificate = PeriodTwoNineDirectionObstructionCertificate(
        ...
        eighth_and_ninth_direction_ranks_mod_two=direction_ranks,
        ninth_residual_length=len(ninth_residual),
        ninth_kernel_length=len(ninth_kernel),
        ninth_degree_two_terms=len(ninth_wedge),
        ninth_degree_two_l1=sum(
            abs(coefficient) for coefficient in ninth_wedge.values()
        ),
        ninth_prior_mod_two_bits=eight.obstruction_bits(ninth_wedge),
        inverse_four_point_action=INVERSE_FOUR_POINT_ACTION,
        inverse_four_point_covector=INVERSE_FOUR_POINT_COVECTOR,
        inverse_four_point_defect=new_defect,
        inverse_four_point_value_mod_two=new_value,
        inverse_four_point_operator_rank=new_rank,
        inverse_four_point_augmented_rank=new_augmented_rank,
    ...
    assert certificate.inverse_four_point_augmented_rank > new_rank
    assert certificate.known_integer_span_obstructed
    return certificate


if __name__ == "__main__":
    print(period_two_nine_direction_obstruction_certificate())
