...
> **Both arms have run. Results: [`results/stable_ac/cov/cov_top3/RESULTS.md`](../../../../results/stable_ac/cov/cov_top3/RESULTS.md).** Headline — abel 640/640 against the node-matched control's 638, 6.9× cheaper on the mean, and every one of the six rows plain greedy cannot take at 100,000 falls to abel's rank-1 pick for 11×–73× less than greedy's own cost. This page stays the *method*; the numbers live there.
...
So the two rows the node-matched control cannot reach are exactly the two the abel rule already takes, for under 8,000 nodes each, and 636–639 are the only genuinely open headline rows.

**Superseded by the run itself.** The 636–639 line above is a *budget-10,000* statement, and the census at 100,000 settles it the other way: all four solve at abel's rank 1, in 14,470–19,340 nodes — just out of reach of a 10,000-node sweep, which is exactly why that sweep saw nothing. Abel takes all six. **The dimensions with real dynamic range are cost and path length**, which is what `summarize()` leads on: median/mean/max `first_solve_nodes` and `path_length` against plain greedy's own, paired over the presentations *both* arms solved, with a win/tie/loss count on each. A 20-presentation dry run at budget 1,000 (stride-sampled, `CHUNKS=32`) already shows the shape — abel mean 66 nodes and mean path 16.9 against plain greedy's 547 and 24.9 on the same rows.
