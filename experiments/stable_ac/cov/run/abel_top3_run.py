"""Stage B of the ms640 abel-top-3 experiment: search the frozen top-3 starts.
...
most ``k * budget`` (3 x 100,000 = 300,000) and usually far less: on subset-60
at budget 1,000 the median cumulative cost to first solve was 18 nodes.

The selection is Stage A's problem and is already frozen on disk
(``abel_top3_manifest``), so this module makes no ranking decisions at all. That
separation is what lets two sessions search disjoint halves of the dataset with
no coordination: shard membership is a pure function of the manifest's row order.

## Sharding

``chunk_index = i`` of ``chunks = N`` runs presentation ``j`` iff
``j % N == i - 1`` — a stride, never a block, because ms640 is difficulty-ordered
and a block split would hand one shard every hard presentation and finish hours
after the other. Each shard writes its own resumable jsonl (``_c{i}of{N}_`` is
part of the filename, hence part of the resume identity — a different row subset
is a different file). ``merge_chunks`` folds them into the canonical unchunked
name once both are complete.
...
The node-matched control is **plain greedy at k x budget**, and it already exists
on disk: ``results/greedy_baseline/greedy_1000000_640_*.jsonl`` truncated at
300,000, because a search at budget B is exactly the first B pops of a longer
one. Do not re-run it. ``summarize`` reads it and prints both arms.

    ACSOLVERX_ALLOW_BIG=1 .venv/bin/python3 -m \
        experiments.stable_ac.cov.run.abel_top3_run --chunk-index 1
"""
...
from experiments.stable_ac.cov.run import abel_top3_manifest as manifest
...
DEFAULTS = {
    ...
    "out_dir": "results/stable_ac/cov/abel_top3",
    "dataset_tag": "ms640",
    "cyclic_reduce": True,
    ...
    "resume": True,
    "chunks": 2,
    "chunk_index": None,             # 1..chunks; None = every presentation, serially
}
...
def _run_prefix(c, n_pres):
    ...
    In: budget, k, the manifest's family tag, base cap, cyclic reduction, the
    ...
    return (f"abeltop3_{c['budget']}_{n_pres}_k{c['k']}_{manifest.FAMILY_TAG}"
...
def _search(c, r1, r2, cap, progress=None):
    ...
    row identical to a slow-mode row. The re-solve gets no callback: its nodes
    are the same nodes, and counting them twice would inflate the rate.
    """
...
def run(root=ROOT, **overrides):
    """Search this shard's presentations, appending one row per search."""
    c = load_config(**overrides)
    groups = manifest.load_manifest(c["manifest"], root=root)
    n_pres = len(groups)
    ...
    label = (f"c{c['chunk_index']}of{c['chunks']}"
             if c["chunk_index"] is not None else "")
    done_pres = sum(1 for _, picks in mine
                    if _finished(done, picks, c["k"]))
    print(f"[abel-top{c['k']}] budget={c['budget']:,} chunk="
          f"{label or 'all'} {len(mine)}/{n_pres} presentations -> "
        ...
        for pres_id, picks in mine:
            ...
            for pick in picks[:c["k"]]:
                ...
                row = run_baseline._build_row(bcfg, pres_id, pick["r1"],
                                              ...
                                              elapsed)
                row.update({
                    "mode": "abel_top3", "rank": pick["rank"], "k": c["k"],
    ...
    print(f"[abel-top{c['k']}] chunk {label or 'all'} done: {n_solved} solving "
          f"searches over {n_seen} searches", flush=True)
    return out_path
...
def merge_chunks(root=ROOT, force=False, **overrides):
    ...
    if not force and (absent or unfinished):
        raise RuntimeError(
            ...
            f"{unfinished[:5]}. Let both shards finish (or pass force=True for "
...
def baseline_at(budget, root=ROOT, pattern=BASELINE_GLOB):
    ...
    1,000,000-node ms640 run read with ``solved and nodes_explored <= budget``.
    ...
    with open(paths[-1]) as fh:
        for line in fh:
            d = json.loads(line)
            out[d["pres_id"]] = bool(d["solved"]) and d["nodes_explored"] <= budget
    return out
...
def verify_overlap(out_path, root=ROOT, budget=None, sweep=OVERLAP_SWEEP):
    ...
    their whole CoV families — so ~180 of this run's searches have a known
    answer before it starts. Greedy search is deterministic and a search at
...
def summarize(out_path, root=ROOT, **overrides):
    """Both arms, scored on the SAME presentations at the same node cost.
    ...
    against another run's denominator.
    """
    c = load_config(**overrides)
    done, n_seen, _ = _read_done(out_path)
    groups = manifest.load_manifest(c["manifest"], root=root)
    matched = c["k"] * c["budget"]

    hit, searched, cum_all, missing = set(), set(), [], []
    for pres_id, picks in groups:
        rows = [done[(pres_id, p["rank"])] for p in picks[:c["k"]]
                if (pres_id, p["rank"]) in done]
        if not rows:
            missing.append(pres_id)
            continue
        searched.add(pres_id)
        cum_all.append((pres_id, sum(r["nodes_explored"] for r in rows)))
    ...
    print(f"[summary] {m}/{n} presentations searched, {n_seen} searches"
          + (f" — {len(missing)} NOT STARTED, every count below is over the "
             f"{m} searched" if missing else ""), flush=True)
    print(f"  abel top-{c['k']} @ {c['budget']:,}      : {len(hit)}/{m}", flush=True)
    print(f"  plain greedy @ {c['budget']:,} (control) : {len(g_1)}/{m}", flush=True)
    print(f"  plain greedy @ {matched:,} (node-matched): {len(g_m)}/{m}", flush=True)
    only_a, only_g = sorted(hit - g_m), sorted(g_m - hit)
    print(f"  discordant vs node-matched: {len(only_a)} only abel "
          f"{only_a[:20] or ''} | {len(only_g)} only greedy {only_g[:20] or ''}",
          flush=True)
    if cum_all:
        vals = sorted(v for _, v in cum_all)
        print(f"  cum_nodes per presentation: median {vals[len(vals) // 2]:,} | "
              f"mean {sum(vals) // len(vals):,} | max {max(vals):,} | "
              f"total {sum(vals):,}", flush=True)
    ...
    return {"solved": hit, "searched": searched, "greedy_matched": g_m,
            "greedy_one": g_1, "cum_nodes": dict(cum_all), "missing": missing,
            "overlap": n_ov, "overlap_violations": bad}
...
def main():
    ...
    ap.add_argument("--summarize", metavar="JSONL",
                    help="print both arms from an existing results jsonl")
    a = ap.parse_args()
