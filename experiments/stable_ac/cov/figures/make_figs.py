"""Regenerate the CoV-vs-baseline figures. Run from anywhere:
    .venv/bin/python3 -m experiments.stable_ac.cov.figures.make_figs
...
Reads and **writes** under `results/stable_ac/cov/graphs/` — every figure write is inside `main()`
under the `__main__` guard, because this module lives in a package and merely importing it used to
overwrite two committed result PNGs.
"""
import json, collections, statistics, math, os
import numpy as np
...
def _repo_root():
    ...
    Never count `os.path.dirname()` levels — see the note in `cov_summary.py`.
    """
...
_ACS = _repo_root()
F=os.path.join(_ACS,"results","stable_ac","cov","covsweep_100_66_subnc2pxysb_mrl24_cyc_s60r6_07_16_26.jsonl")
OUT=os.path.join(_ACS,"results","stable_ac","cov","graphs"); BUDGET=100
