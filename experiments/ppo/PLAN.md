...
Answers to what this section used to ask for: the GPU is an **A100**; `torch` is installed locally and `jax`/`flax`/`orbax`/`distrax` are not, so the cross-framework parity gate is run against a throwaway venv that carries all of them (see step 1 — it is closed). Colab's image ships torch and jax; `flax`, `orbax-checkpoint`, `distrax` and `wandb` are installed by SETUP. `distrax` goes on its own `pip` line because it drags in tensorflow-probability and is the one install here that can plausibly fail — `run_ppo._ensure_distrax` stands in a `Categorical` shim when it does, verified bit-identical to distrax 0.1.9 on the real checkpoint. W&B goes to project `acsolver`, entity `avigyapaudel045-aisc`.
