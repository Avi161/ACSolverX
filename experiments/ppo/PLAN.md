...
Answers to what this section used to ask for: the GPU is an **A100**; `torch` is installed locally and `jax`/`flax`/`orbax`/`distrax` are not, so the cross-framework parity gate is run against a throwaway venv that carries all of them (see step 1 — it is closed). Colab's image ships torch and jax; SETUP installs `flax`, `orbax-checkpoint` and `wandb`. **`distrax` is deliberately not installed** (`INSTALL_REAL_DISTRAX = False` in CONFIG): `network.py` only ever calls `distrax.Categorical(logits=...)` and reads `.logits`, while the package itself depends on **`tfp-nightly`** — a build that changes daily, landing next to the pinned TensorFlow already on the image — so installing it risks breaking a working runtime to gain one wrapper class. `run_ppo._ensure_distrax` stands in a six-line `Categorical` shim instead, verified bit-identical to distrax 0.1.9 against the real `610model` (`3.7933267034162554e-05` from both, every other gate field equal). The parity report records which one ran, under `distrax: installed | shim`. W&B goes to project `acsolver`, entity `avigyapaudel045-aisc`.
...
0. **The smoke** — `SMOKE_RUN = True` in CONFIG (the shipped default). Runs `convert` → `parity` → a wall-clock-bounded slice of `beam_upstream` → `report`, in `SMOKE_SECONDS` (300) plus parity. The beam stays at **production width**: the budget stops it between presentations, so every row it writes is a full 1024 × 150 decode that the full run resumes past and keeps, and its seconds/presentation extrapolates directly to 1190 — neither is true of a narrowed beam. `report` writes `smoke_report.json` to Drive with the GPU, the parity verdict, the solve rate, measured seconds/row with its spread, the projected full-run hours, and a `verify_beam` certificate check of every solved row. Set `SMOKE_TRAIN = True` to add two throwaway training updates and exercise the optimiser, checkpoint and mirror as well.

1. **`parity`** — minutes. Same weights, same batch, JAX vs torch, TF32 off. **Now closed locally** (in a throwaway venv carrying torch + jax + flax + distrax + orbax at once, against the real `610model`): the two action masks are identical over 10,528 legal actions in 256 states, `max|dlogprob| = 3.79e-05`, `max|dvalue| = 2.86e-06`, and the argmax and top-10 agree on 256/256 — so Colab is a confirmation on CUDA rather than the first attempt. Two things had to be right for that number to mean anything. The comparison is over **log-probabilities on legal actions**: `distrax.Categorical(logits=l).logits` returns `log_softmax(l)`, so comparing it against raw torch logits differs by the per-row logsumexp — 5.98 on this checkpoint, which reads as a broken port and is not one (its signature is a large maximum with 2.6e-06 spread *within* each row). And the masked entries are excluded because they are a −1e9 sentinel where a float32 ulp is 64, so their difference measures rounding on a constant; the masks are compared directly instead, which is the stronger check. The stage is **diagnostic and cannot stop the ladder** — nothing downstream imports JAX, so any failure under it is logged and stepped over.
