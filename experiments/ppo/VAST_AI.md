...
*Alternative:* launch the instance with Jupyter and run `experiments/notebooks/ppo/ppo_arm_s20mk2.ipynb` unchanged. Its SETUP cell already has a non-Colab path (it walks up to the repo root and sets `MIRROR_DIR = None`), so the only edit is trimming `STAGES` to `["train", "bench60", "report"]`. This sidesteps the config question entirely and is the better choice if you would rather not touch a shell.
