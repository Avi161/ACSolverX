...
| cap-255 correctness, not speed: path bytes decoded unsigned; an explicit `reserve_states` allocated exactly; input beyond the cap and a reservation past int32 both rejected up front | `hcompact.py` (commit `d01a315f`, `ENGINE_MEM_GEN` 5 -> 6) | no speed claim and none measured; `plan_memory` returns the SAME reservation as gen 5 (2,140,016,900 at cap 64 / 10M) because the planner already added the `4*(cap+1)^2` pad the solver was adding a second time | oracle gate PASS (60 rows x 1,000 vs the Python reference) and twin gate PASS (6 rows x 30,000 vs `frozen2`), 2026-09-06. Cut positions are stored by the same wrapping arithmetic as before, so the bytes on disk are unchanged at cap <= 127 and only the decode differs; the two guards raise on inputs the old code accepted and then corrupted |
