"""L + w·S (smaller mean block) weight sweep at budget 1,000 on subset-60.

``S`` = mean run length of the thinner generator over the *pair*: concatenate
all x-run lengths from r1 and r2, same for y, take each generator's mean,
then ``S = min(mean_x, mean_y)``. Mean S on this set is ~1.09; RECOMMENDED
uses coefficient 8.458.

No K / MK / xyimb. Cap 24. Compared to shipped length-only and RECOMMENDED.

    python3 -m experiments.heuristic_search.runners.run_s_b1k
"""
...
OUT_CSV = os.path.join(ROOT, "results/comparison/s_b1k_subset60.csv")
OUT_MD = os.path.join(ROOT, "results/comparison/S_B1K.md")

BUDGET = 1_000
CAP = 24

# S ≈ 1.09 here (range ~1.0–1.25); RECOMMENDED uses 8.458. Bracket it and
# probe higher so contribution can match the better one-feature peaks
# (L+8·MK contrib ~21 ⇒ w_S ~19).
WEIGHTS = (2.0, 4.0, 8.458, 12.0, 16.0, 20.0, 24.0, 32.0)


def _s_pair(r1, r2):
    """Match hlab.phi's S: pool runs across both relators, then min mean."""
...
def make_priority(w):
    def priority(r1, r2):
        L, s = _s_pair(r1, r2)
        return L + w * s
    priority.__name__ = f"L+{w:g}*S"
    return priority


def _tag(w):
    return f"s_w{f'{w:g}'.replace('.', '_')}"
...
def run():
    ...
    hsearch("X", "Y", 20, priority=make_priority(8.458), max_relator_length=CAP)
    ...
    lines = [
        ...
        "",
        "Priority `L + w * S`, where `S` is the **smaller mean block** over "
        "the pair: pool all x-runs from `r1` and `r2`, same for y, take each "
        "generator's mean run length, then `S = min(mean_x, mean_y)`. Cap 24. "
        "No K / MK / xyimb.",
        "",
        "Mean S on these starts is ~1.09 (almost all in `[1.0, 1.25]`); "
        "RECOMMENDED's S coefficient is 8.458. Grid: "
        "`{2, 4, 8.458, 12, 16, 20, 24, 32}`.",
        "",
    ...
    for w in WEIGHTS:
        ...
        summary[tag] = n
        lines.append(fmt_row(f"`L + {w:g}·S`", n, mn, mdn, mp, mdp))
    ...
    lines.extend([
        ...
        "## Verdict",
        "",
        f"- Best weight: `L + {best_w:g}·S` → **{best_n}/60**",
        f"- vs length-only 29/60 and full RECOMMENDED 43/60.",
        f"- Nearby one-feature peaks: `L+8·MK` = 38/60, `L+8·K` = 37/60, "
        f"`L+10·K_den` = 32/60.",
        "",
    ...
    lines.extend([
        ...
        f"- Table: [`s_b1k_subset60.csv`](s_b1k_subset60.csv)",
    ...
    print(f"VERDICT: L + {best_w:g}·S → {best_n}/60")
    return rows
