"""L + w·Bmax (larger mean block) weight sweep at budget 1,000 on subset-60.

``Bmax = max(mean_x, mean_y)`` — the thicker generator's mean run length
over the pair (pool runs from r1 and r2 by generator, then take the max
mean). Contrast with ``S = min(mean_x, mean_y)`` in ``run_s_b1k``.

Mean Bmax on this set is ~3.23 (vs S ~1.09). Grid is contribution-matched
to the S sweep: ``w_B ≈ w_S · μ_S/μ_B``. Cap 24; no K / MK / xyimb.

    python3 -m experiments.heuristic_search.runners.run_bmax_b1k
"""
...
OUT_CSV = os.path.join(ROOT, "results/comparison/bmax_b1k_subset60.csv")
OUT_MD = os.path.join(ROOT, "results/comparison/BMAX_B1K.md")

BUDGET = 1_000
CAP = 24

# Bmax ≈ 3.23; S-sweep peak was w_S=24. Contribution-matched grid plus a
# bit of range on either side.
WEIGHTS = (1.0, 2.0, 3.0, 4.0, 6.0, 8.0, 10.0, 12.0, 16.0)


def _bmax_pair(r1, r2):
    """Match hlab.phi's Bmax: pool runs across both relators, then max mean."""
    n1, _x1, _y1, xr1, yr1, _nx1 = word_stats(r1)
    n2, _x2, _y2, xr2, yr2, _nx2 = word_stats(r2)
    ...
    if not xs or not ys:
        ...
        b = max(mx, my)
    return float(n1 + n2), float(b)


def make_priority(w):
    def priority(r1, r2):
        L, b = _bmax_pair(r1, r2)
        return L + w * b
    priority.__name__ = f"L+{w:g}*Bmax"
    return priority


def _tag(w):
    return f"bmax_w{f'{w:g}'.replace('.', '_')}"
...
def run():
    ...
    hsearch("X", "Y", 20, priority=make_priority(8.0), max_relator_length=CAP)
    ...
    lines = [
        ...
        "",
        "Priority `L + w * Bmax`, where `Bmax = max(mean_x, mean_y)` — the "
        "**thicker** generator's mean run length over the pair (same pooling "
        "as `S`, but `max` instead of `min`). Cap 24. No K / MK / xyimb.",
        "",
        "Mean Bmax on these starts is ~3.23 (vs S ~1.09). Weight grid is "
        "contribution-matched to the S sweep (`w_B ≈ w_S · μ_S/μ_B`).",
        "",
    ...
    for w in WEIGHTS:
        ...
        summary[tag] = n
        lines.append(fmt_row(f"`L + {w:g}·Bmax`", n, mn, mdn, mp, mdp))

    best_n = max(summary[_tag(w)] for w in WEIGHTS)
    best_ws = [w for w in WEIGHTS if summary[_tag(w)] == best_n]
    ...
    lines.extend([
        ...
        "",
        f"- Best weight(s): "
        + ", ".join(f"`L + {w:g}·Bmax`" for w in best_ws)
        + f" → **{best_n}/60**",
        f"- vs length-only 29/60 and full RECOMMENDED 43/60.",
        f"- Contrast: best `L+w·S` (`min`) was **37/60** at w=24 "
        f"([`S_B1K.md`](S_B1K.md)). Clustering already found larger mean "
        f"block near-useless as a classifier (AUC ≈ 0.47); this checks it "
        f"as a search ordering.",
        "",
    ...
    lines.extend([
        ...
        f"- Table: [`bmax_b1k_subset60.csv`](bmax_b1k_subset60.csv)",
    ...
    print(f"VERDICT: best={best_ws} → {best_n}/60")
    return rows
