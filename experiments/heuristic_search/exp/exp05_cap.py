...
with a lower cap is a tautology rather than evidence.
    ...
    python3 -m experiments.heuristic_search.exp.exp05_cap
"""
import json
import os
import sys
...
from experiments.heuristic_search.core.lab import evaluate, rank, read  # noqa: E402
...
def top_configs(n=N_TOP):
    ...
    known = {}
    from experiments.heuristic_search.exp.exp02_single import configs as c2
    known.update({cfg_name(c): c for c in c2()[0]})
    try:
        from experiments.heuristic_search.exp.exp03_segments import configs as c3
        known.update({cfg_name(c): c for c in c3()[0]})
    except Exception:
        pass                                   # EXP-03 has not run yet; its arms simply are not offered
    try:
        from experiments.heuristic_search.exp.exp04_multi import configs as c4
