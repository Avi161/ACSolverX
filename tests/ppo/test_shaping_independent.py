...
the other"), written without reading `heuristics.word_stats`'s body for guidance beyond the
English definition, and using a structurally different algorithm (rotate to a run boundary,
then plain linear run-length encoding) instead of the reference's "encode linearly, then
merge the seam". `shaping.features`, `heuristics.phi` and this reference are then
cross-checked against EACH OTHER on random and degenerate relator pairs, rather than
...
def hsearch_feats(r1, r2):
    """(L, K, MK, S) from `heuristics.phi`, which returns all seventeen features in
    FEATURES order (L, K, MK, S sit at indices 0, 4, 5, 7). The int() casts reproduce
    the old `hsearch.feats` contract exactly: three int counts and a float mean."""
    v = phi(r1, r2)
...
def ref_blocks(word):
    ...
    Never calls `heuristics.word_stats`. Route: rotate the word to start at a run boundary
    (a position whose generator differs from its cyclic predecessor), then do ordinary
    LINEAR run-length encoding. Because the rotation point is chosen to be a boundary, the
    linear encoding's first and last runs are never the same generator, so no seam-merge
    step is needed -- a different mechanism than the reference's "encode linearly, then
    merge the two ends", which should agree with it only because both are correct.
    """
    n = len(word)
    if n == 0:
        return []
...
# ============================================================================================
# shaping.py -- three-way cross-check: ref_feats, heuristics.phi, shaping.features
# ============================================================================================
