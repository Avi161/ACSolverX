...
def test_the_recommended_heuristic_is_not_an_arm_and_must_not_become_one():
    """User directive, standing: `hsolve.RECOMMENDED` is never used in this work.

    It was tuned on ~60 presentations and its xyimb weight flips sign between the two
    shipped configs. This asserts against the live `RECOMMENDED` rather than a copy of
    its numbers, so it keeps holding if that config is retuned -- and it fails loudly
    if anyone adds those weights back as a second arm.
    """
    banned = {w for k in ("L", "K", "MK", "S")
              for w in [RECOMMENDED["segments"][-1]["w"][k]] if w != 1.0}
