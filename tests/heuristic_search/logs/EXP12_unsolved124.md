...
Every class from `unsolved_124_aca_classes.csv`, started from each of its **distinct signed-permutation relabels** (980 starts over 124 classes — not 992, because three classes are symmetric enough to be fixed by half the permutations), at budget 1000 with cap 64. A relabel is the same presentation under a rename of the generators, so it is the same problem — but the greedy reads strings, so each image is a different search and any one solving settles the class.
