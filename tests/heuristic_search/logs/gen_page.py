...
<section id="hyper">
  ...
  <div class="panel" style="margin-top:20px"><div class="phead"><h2>Which ordering to run at Colab scale</h2>
    ...
    <p style="margin:10px 0 0;font-size:13px;color:var(--ink-2)">A bench66 run at 100,000 nodes is in flight to settle whether that tail keeps widening at real scale. The campaign its answer gates &#8212; 10<sup>6</sup> nodes on the 124 unsolved classes, treatment arm only (the baseline&#8217;s 0/124 is already known from the 10<sup>7</sup> data that defined the set) &#8212; was specified <b>before</b> the answer landed, in <code>tests/heuristic_search/logs/SCALE_RUN_PLAN.md</code>, so the decision procedure is on record either way. And the ordering now runs on the packed-arena solver (<code>hcompact</code>, ~78&#8202;B/state, pop-identical across an 880-search cross-check): a 10<sup>6</sup> search reserves ~7&#8202;GB instead of holding ~24, which moves a 51&#8202;GB machine&#8217;s ceiling from ~2M to ~5M nodes.</p></div>
  </div>
