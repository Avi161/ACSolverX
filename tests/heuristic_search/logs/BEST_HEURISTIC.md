...
**Memory.** `hsolve` uses the normal solver's string-keyed dict, not the compact packed arena. Measured worst-case (the unsolved 124 at cap 48, full budget burned): **36.5 kB per node popped with the certificate map, 24 kB with `keep_path=False`** (1.53×) — so on a 51 GB machine, 500k nodes is the safe ceiling with the map and 10⁶ fits without it (~24 GB). The low-memory mode is result-pure — every scored field identical, pinned by `tests/heuristic_search/test_hsolve.py` — and the certificate of anything that solves is recovered exactly by a deterministic re-run, which `run_ab.py` does automatically (`KEEP_PATH = False` in the notebook's CONFIG cell). Porting the ordering into the compact packed arena remains the right move beyond 10⁶.
