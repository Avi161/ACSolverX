...
- **Actual-base relation module evaluated for both canonical endpoints:** an
  ...
  quotient transfer.
- **Tag-coupled module gate isolated:** along the fixed branch-B quotient
  path, residual transport is affine, `e=J_P*b+kappa_P`, with homogeneous
  part `J_P` tame and invertible. If `L_e` is the module generated by
  `[A],[E_B],[H_B*v^-1]`, then `L_e=R` exactly when `kappa_P` vanishes in
  `(R/L_e)^3`. Thus homogeneous invertibility and `Wh(G_-)` do not close
  the tag-coupled endpoint. The terminal obligation for this lift is one
  curvature filling or one surviving cokernel class. Freeze it there: do not
...
- **The specific kernel is one named syzygy:** writing the second coordinates
