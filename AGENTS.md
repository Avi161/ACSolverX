...
- [WORKS] A theoretically justified proof experiment may exceed 60 seconds only through the guard's explicit `--long-run` mode with `60 < --timeout-seconds <= 600`, after the identical command succeeds in its `ACSOLVERX_PROOF_PHASE=preflight` phase with `0 < --preflight-seconds <= 60`. Only the root/controller may launch it, in the foreground; subagents may prepare theory or code but must not start the computation. The singleton lock spans both phases, and exactly one guarded experiment and one numerical CPU thread are permitted.
- [WORKS] Set `1 <= --progress-seconds <= 60`, report progress to the user within every 60-second window, and stop early on macOS thermal pressure, sustained whole-process-group CPU above the one-thread allowance, an escaped descendant, controller loss or phase-leader reparenting, or any unexpected process-tree state. Process inspection may request only PID, PPID, PGID, CPU percentage, state, and executable name. The bounded internal thermal helper alone calls Foundation with fixed arguments; thermal and `ps` inspection share one absolute sample deadline, and a helper deadline emits only the sanitized safety transition before cleaning its exact group. Signal only the exact active phase group (or the helper's exact group while cleaning the helper), with `SIGTERM` before a bounded grace period and `SIGKILL` only if that same group survives. Never signal the controlling shell, Codex/ChatGPT, terminals, applications, or an ambiguous escaped PID.
- [WORKS] Interpret long-run status `73` as a held singleton lock, `124` as experiment timeout, `125` as failed or timed-out preflight with no experiment launch, `126` as a safety or cleanup failure, and `2` as invalid limits; otherwise preserve the experiment's completion status.
- [WORKS] After completion or interruption, verify the exact child group is absent, the guard lock is absent, and no stale project Python, pytest, uv, or Numba process remains. Never rerun an unchanged timed-out experiment. Record timeout or nonzero completion only as a bounded failure—not evidence against AC or stable AC—and immediately commit, bind the mandatory push log, and push the result before continuing.
