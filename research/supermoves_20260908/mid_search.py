"""Research search with budgeted certified completion at pop or discovery.

Based on experiments/search/heuristic_1k.py. With probes off, ordering and
search outcomes match that engine. Recognition cost is recorded separately.
"""
import heapq
import numpy as np
...
def mixed_search(pair, arm, budget=1000, cap=48, w_weight=None,
    ...
    def result(solved=False,key=None,winner=None,macro=None,tail=None):
        steps,states=[],[]
        if solved:
                ...
                if use_bs_preflight:
                    ...
                    if (bs_escape_macro_budget and not escape_macros and rejected
                        ...
                        continuation = mixed_search(
                            ...
                            use_bs_preflight=True, bs_escape_weight=4.)
