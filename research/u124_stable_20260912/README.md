...
4. Wave 2 (guarded, 60 s): `python3 scripts/run_proof_guarded.py --timeout-seconds 60 -- python3 research/u124_stable_20260912/code/wave2_structural.py`
   ...
   Do not re-run the 579,870 nine-config census. C36:
   `.../code/c36_yxxyx_family.py` (full census) or `--annotate-existing`.
   Do not re-run the 1.55M nine-config census. C37:
   `.../code/c37_x_exact_l1.py` (full census) or `--annotate-existing`.
   Do not re-run the 11,804 exact-L1 census. C38:
   `.../code/c38_yxxxyx_family.py` (full census) or `--annotate-existing`.
   Do not re-run the 1.80M nine-config census. C39:
   `.../code/c39_yxxyxyxxx_family.py` (full census) or `--annotate-existing`.
   Do not re-run the 1.56M nine-config census. C40:
   `.../code/c40_yyxxxyxx_x_eq_c.py` (full census) or `--annotate-existing`.
   Do not re-run the 332,199 nine-config census. C41:
   `.../code/c41_yyxxxyxx_x_eq_cinv.py` (full census) or `--annotate-existing`.
   Do not re-run the 164,475 nine-config census. C42:
   `.../code/c42_yyxxxyxx_y_eq_cinv.py` (full census) or `--annotate-existing`.
   Do not re-run the 183,924 nine-config census.
...
6. Advisor reviews live in `ingest/advisor_*.md`. Theory drafts live in
   ...
   `ξ`; Gate 2 on `Q'` has even `L1`, so odd `k` is impossible, and
    ...
    length 11). Plan advisor REVISE applied. Advisor APPROVE. C38: listed
    `YXXXyxYxx` rows (`y ≡ D^{-1}`, `L1=1`); k=3 nine-config misses
    ...
    Plan advisor APPROVE. Advisor APPROVE. C40: two listed `YYXXXyxx`
    rows with `C_ab=(1,0)` (`x ≡ C` in abelianization, `L1=1`); k=3
    nine-config misses (`332,199` products, observed minima 9 and 11).
