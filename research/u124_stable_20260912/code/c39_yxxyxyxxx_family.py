#!/usr/bin/env python3
"""C39: YXXyXYxxx family — y ≡ D^{-1} (L1=1) on six listed rows, then k=3 ncl.

Unused listed shared-donor D = YXXyXYxxx, exponent (0,−1), cyclic length 9.
Six best-table rows, all with |p|=1 so det(D,C)=p=±1. On those listed
...
orientation). Parallel to C38; disjoint donor/row presentation pairs,
not a disjoint companion-word pool. Not the last unused listed donor.
...
JSON_PATH = OUT / "c39_yxxyxyxxx_family.json"

DONOR = "YXXyXYxxx"
ALPHABET = "xXyY"
POSITIVE_Y = c29.POSITIVE_Y
ORIENTATION = c35.C35_ORIENTATION
if ORIENTATION != c35.C35_ORIENTATION:
    raise RuntimeError("C39 must use C35 orientation")
if ORIENTATION == c35.C31_ORIENTATION:
    raise RuntimeError("C39 must not use C31 orientation")
ROW_IDS = (
    "aca_45",
    "aca_61",
    "aca_77",
...
EXPECTED_COMPANIONS = {
    ...
    "aca_61": "YYYYYXXXYxx",
    "aca_77": "YYYYYYXXXYxx",
    "aca_83": "YYYYYYYXXyxxx",
    "aca_107": "YYYYYYYXXXYxx",
    "aca_114": "YYYYYYYYXXyxxx",
}
EXPECTED_C_EXP = {
    "aca_45": (-1, -5),
    "aca_61": (-1, -6),
...
EXPECTED_TYPED_TOTAL = 1_563_690
OVERLAPPING_COMPANIONS = {
    "YYYYYYXXXYxx": ("aca_77", "aca_75"),
    "YYYYYYYXXXYxx": ("aca_107", "aca_84"),
}
...
def y_combo_from_listed_c_exp(p: int, q: int) -> tuple[int, int, int]:
    ...
    """
    if abs(p) != 1:
        raise ValueError("C39 combo (-1,0) is for listed unimodular C_ab with |p|=1")
    return -1, 0, 1
...
def abelian_row(name: str, companion: str) -> dict:
    if name not in ROW_IDS:
        raise ValueError(f"{name} is not a listed C39 row")
...
def three_factor_row(name: str, companion: str) -> dict:
    ...
    if rec["orientation"] != c35.C35_ORIENTATION:
...
def planted() -> dict:
    ...
    return {
        ...
        "ok": (
            ...
            and witness_ok
            and rec["orientation"] == c35.C35_ORIENTATION
        ),
...
def summarize(abelian: list[dict], three: list[dict], ctrl: dict) -> dict:
    ...
    if n_typed != EXPECTED_TYPED_TOTAL or n_products != EXPECTED_TYPED_TOTAL:
        raise RuntimeError(
            f"C39 typed/product totals {n_typed}/{n_products} != {EXPECTED_TYPED_TOTAL}"
        )
    if not all(row["n_products_equals_typed"] for row in three):
        raise RuntimeError("C39 per-row n_products != n_typed_tuples")
    ids = [row["id"] for row in abelian]
    if tuple(ids) != ROW_IDS:
        raise RuntimeError(f"C39 row ids {ids} != {ROW_IDS}")
    return {
        "n_rows": len(abelian),
        "donor": DONOR,
        ...
        "three_all_orientation_c35": all(
            row["orientation"] == c35.C35_ORIENTATION for row in three
        ),
...
def notes_from_summary(summary: dict) -> list[str]:
    ...
    return [
        "D = YXXyXYxxx has exponent (0,−1). On the six listed |p|=1 rows, y has unique combination (−1,0), L1=1.",
        ...
        "Disjoint donor/row presentation pairs from C38. Companion words overlap C38. Not the last unused listed donor.",
        ...
        "Observed minimum length is a C39 census statistic, not a theorem and not a comparison with C38/C29/C33.",
...
def main() -> dict:
    ...
    best = load_best()
    ctrl = planted()
    print("c39 planted", ctrl["ok"], flush=True)
    abelian = []
    three = []
    for name in ROW_IDS:
        row = best[name]
        ...
        three.append(rec)
        print(
            f"c39 {name} products={rec['n_products']} min_len={rec['min_len']} "
            f"found={rec['found']}",
            flush=True,
        )
