#!/usr/bin/env python3
"""C45: one YXXXyxx row with y ≡ C^{-1} in abelianization (L1=1), then k=3 ncl.

Listed shared-donor D = YXXXyxx, exponent (−1,0), cyclic length 7.
C35 already searched x ≡ D^{-1} k=3 on six rows with C35 orientation.
Only aca_117 has C_ab=(0,-1) with unique y-combo (0,-1), so L1=1 and
y ≡ C^{-1} abelianly. This is not free equality, not every companion
of D, not a six-row YXXXyxx census, and not a re-run of C35.

The other five C35 rows have y with L1 in {2,3,4} and are not C45.

Even k is abelian-impossible (C24.1). k=1 is blocked by |C|=7. k=3 is
...
JSON_PATH = OUT / "c45_yxxx_y_eq_cinv.json"
...
if ORIENTATION != c35.C31_ORIENTATION:
    raise RuntimeError("C45 must use C31 orientation")
if ORIENTATION == c35.C35_ORIENTATION or ORIENTATION == c40.C40_ORIENTATION:
    raise RuntimeError("C45 must not use C35 or C40 orientation")
ROW_IDS = ("aca_117",)
EXPECTED_COMPANIONS = {
    "aca_117": "YYYXyyx",
}
...
EXPECTED_TYPED_TOTAL = 83349
C35_ACA117_TYPED = 83349


def y_combo_from_listed_c_exp(p: int, q: int) -> tuple[int, int, int]:
    """Unique y combo against D_ab=(-1,0) when C_ab=(0,-1).

    Not a claim about every companion of this donor.
    """
    if (p, q) != (0, -1):
        raise ValueError("C45 combo (0,-1) is for listed C_ab=(0,-1) companion")
    return 0, -1, 1
...
def abelian_row(name: str, companion: str) -> dict:
    if name not in ROW_IDS:
        raise ValueError(f"{name} is not a listed C45 row")
    ...
    x = rows["x"]
    if x["combo"].get("a") != -1 or x["combo"].get("b") != 0 or x["L1"] != 1:
    ...
    return {
        ...
        "x_combo": x["combo"],
        "x_L1": x["L1"],
        "x_equiv_Dinv_in_abelianization": True,
        "x_k3_was_c35_not_part_of_c45": True,
        "do_not_rerun_c35": True,
        "k1_len_obstruction": c_cyc > 1,
        "k1_blocked_by_C_cyc_len_7": c_cyc == 7,
        "even_k_forbidden": y["L1"] == 1,
        "combo_restricted_to_listed_C_ab": True,
        "development_panel_n2_wall_is_disclosure_not_a_solve": True,
        "not_a_c16_reproof": True,
        "equal_typed_size_does_not_identify_c35_aca117": True,
        "targets": rows,
    }
    ...
    rec.update(
        {
            ...
            "equal_typed_size_does_not_identify_c35_aca117": typed == C35_ACA117_TYPED,
...
def summarize(abelian: list[dict], three: list[dict], ctrl: dict) -> dict:
    ...
    if n_typed != EXPECTED_TYPED_TOTAL or n_products != EXPECTED_TYPED_TOTAL:
        raise RuntimeError(
            f"C45 typed/product totals {n_typed}/{n_products} != {EXPECTED_TYPED_TOTAL}"
        )
    if not all(row["n_products_equals_typed"] for row in three):
        raise RuntimeError("C45 per-row n_products != n_typed_tuples")
    ids = [row["id"] for row in abelian]
    if tuple(ids) != ROW_IDS:
        raise RuntimeError(f"C45 row ids {ids} != {ROW_IDS}")
    return {
        "n_rows": len(abelian),
        "donor": DONOR,
        ...
        "all_pair_det_plus_one": all(row["pair_det"] == 1 for row in abelian),
        ...
        "all_k1_blocked_by_C_cyc_len_7": all(
            row["k1_blocked_by_C_cyc_len_7"] for row in abelian
        ),
        ...
        "all_x_k3_was_c35_not_part_of_c45": all(
            row["x_k3_was_c35_not_part_of_c45"] for row in abelian
        ),
        "do_not_rerun_c35": True,
        "all_even_k_forbidden": all(row["even_k_forbidden"] for row in abelian),
        "combo_restricted_to_listed_C_ab": True,
        "not_six_row_yxxx_census": True,
        "not_lumped_with_c35_or_other_y_l1": True,
        "development_panel_n2_wall_is_disclosure_not_a_solve": True,
        "not_a_c16_reproof": True,
        ...
        "three_min_len_is_product_word_not_pair_total": True,
        ...
        "c35_used_c35_orientation_on_this_row_for_x": True,
        "c44_touched_this_row_under_length_descent": True,
        "equal_typed_sizes_do_not_identify_censuses": True,
        "equal_typed_size_83349_does_not_identify_c35_aca117": True,
        "x_k3_was_c35_not_part_of_c45": True,
...
def notes_from_summary(summary: dict) -> list[str]:
    ...
    return [
        "D = YXXXyxx has exponent (−1,0). On listed C_ab=(0,-1) row aca_117, y has unique combination (0,-1), L1=1, and y ≡ C^{-1} in abelianization.",
        "That is not free equality y=C^{-1}, not every companion of D, not a six-row YXXXyxx census, and not a re-run of C35.",
        "C35 already did x ≡ D^{-1} k=3 on this donor/row with C35 orientation. C45 does not re-run C35.",
        "Even k is abelian-impossible. k=1 is blocked by |C|=7. k=3 uses C31 orientation R^+=C^{-1}, S=D.",
        ...
        "Observed minimum is freely reduced product-word length, not a pair total, not a drop from BEST total 14, and not a C10 finish.",
        "Completeness is all 83,349 typed tuples in this bounded pool.",
        "Equal typed size 83,349 does not identify C35's aca_117 x-census (different orientation and targets).",
        "Disjointness is relative to prior typed donor-family censuses. C28 and C44 touched this row under different predicates.",
        "aca_117 development-panel / n=2-wall status is a disclosure, not a C16 re-proof.",
...
def main() -> dict:
    ...
    best = load_best()
    ctrl = planted()
    print("c45 planted", ctrl["ok"], flush=True)
    abelian = []
    three = []
    for name in ROW_IDS:
        row = best[name]
        _donor, companion = split_pair(row["r1"], row["r2"])
        if (row["r1"], row["r2"]) != ("YYYXyyx", "YXXXyxx") and (
            row["r2"],
            row["r1"],
        ) != ("YYYXyyx", "YXXXyxx"):
            raise RuntimeError(f"{name}: unexpected displayed pair {row['r1']} {row['r2']}")
        ab = abelian_row(name, companion)
        abelian.append(ab)
        rec = three_factor_row(name, companion)
        three.append(rec)
        print(
            f"c45 {name} products={rec['n_products']} min_len={rec['min_len']} "
            f"found={rec['found']}",
            flush=True,
        )
    ...
    print("c45 YXXXyxx y ≡ C^{-1} in abelianization")
