...
def bfs(r1, r2, cap, max_states=1_000_000, stop_when_solved=True,
    ...
    cap = int(cap)
    if cap < 1 or cap > MAX_LEN:
        raise ValueError("cap must be in 1..%d" % MAX_LEN)
    ...
    if collect_states:
        res["_state_keys"] = sorted(
            "%s,%s" % (key_to_str(int(nk1[t])), key_to_str(int(nk2[t])))
