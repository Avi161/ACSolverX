"""Conjugate dictionary exchange with a reserved central-power pinch lane."""
...
def probe(words, remaining, *, audits=None):
    ...
    for state, path in seeds:
        if used == remaining:
            break
        allowance = min(256, remaining - used)
        local_audit = []
        results, cost = central.probe(state, max(0, allowance - 48), audits=local_audit)
        used += cost
        ...
        if audits is not None:
            audits.append({'seed_length': length(state), 'central_models': local_audit,
                           'central_candidates': len(results)})
