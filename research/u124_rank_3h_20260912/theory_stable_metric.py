"""Retain x and a new stable spelling h=y^p x y^q."""
...
def _parameters(root, model, bound):
    ...
    seen = set()
    for pair in chain(priority, product(range(-bound, bound + 1), repeat=2)):
        if pair != (0, 0) and max(map(abs, pair)) <= bound and pair not in seen:
            seen.add(pair)
            yield pair
...
def probe(words, remaining, *, audits=None):
    ...
    for root in corridor.roots(current):
        for donor in range(3):
            ...
            bound = max(1, abs(root.exponent)) * max_cost
            if bound < 0:
                continue
            for left, right in _parameters(root, model, bound):
                if used == remaining:
                    return candidates, used
                used += 1
                power_cost = len(corridor.shortest_power(left, root)) + len(corridor.shortest_power(right, root))
                if power_cost > max_cost:
                    continue
