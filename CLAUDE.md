...
- Experiment code lives under `experiments/` ([map](experiments/README.md)); run outputs go to `results/` ([map](results/README.md)); the frozen evaluation set is `benchmark/` at the repo root ([map](benchmark/README.md)) — derived from `results/greedy_baseline/` but consumed as an *input* by two dozen files. **Results never live beside the code that made them, and never under `tests/`.** Baseline greedy pipeline: `experiments/search/greedy_baseline.py` (solver, adapted from `greedy_search.ipynb`), `experiments/run_baseline.py` (jsonl + resume + W&B), `experiments/wandb_tracking.py` (W&B identity + charts), `experiments/notebooks/greedy_baseline.ipynb` (CONFIG/SETUP/RUN). Also `experiments/analysis/` (the benchmark) and `experiments/equivalence_classes/` (`lib`/`search`/`pipeline`/`verify`/`phases`).
...
- **Scaled nb2 = `experiments/stable_ac/cov/ladder/mu_ladder_big.py`** (+ `experiments/notebooks/stable_ac/mu_ladder_big.ipynb`, `verify_mu_ladder.py`; tests `tests/stable_ac/test_mu_ladder_big.py`): same `climb_one` semantics (regression-pinned), plus chunked spawn parallelism, deterministic per-class budgets (`rungs`/`max_orbits`; the time knob is only a runaway backstop), and full provenance — every accepted orbit carries its concrete parent pair + `(z, iso_gen, iso_index)` and replays independently via `verify_mu_ladder` (`--level full` re-derives `aut_canon` too). `stop_mu` defaults to 12, never 13. Canonicalisation runs on `autcanon_fast.py`, the numba twin of `aut_canon` (~15× end-to-end, rows identical — pinned by `tests/stable_ac/test_autcanon_fast.py`); the verifier stays on slow `aut_canon` so every row is a fast-vs-slow cross-check.
...
**All test suites live under one root `tests/`, nested by area** (`tests/greedy/`, `tests/stable_ac/`, `tests/heuristic_search/`, `tests/clustering/`, `tests/analysis/`, `tests/equivalence_classes/`, `tests/wandb_tracking/` — all seven mapped in [`tests/README.md`](tests/README.md)), with the shared `conftest.py` at `tests/`. A bare `pytest` collects them all (`pytest.ini` `testpaths = tests`). The test SUPPORT code they import — `experiments/greedy_tests/{spec,fixtures,adapters.py,tools,golden}` — deliberately stays under `experiments/` because production modules import it too; do not move it.

### ⛔ MANDATORY after ANY change to the greedy pipeline

**Any** edit to `experiments/search/greedy_baseline.py`, `experiments/run_baseline.py`, `experiments/notebooks/greedy_baseline.ipynb`, or anything under `tests/greedy/`, `tests/stable_ac/`, `tests/analysis/` or `experiments/greedy_tests/` (the spec/fixtures support) **must** be followed by:

```bash
.venv/bin/python3 -m pytest tests/greedy tests/stable_ac tests/analysis -q            # after every change (~3–5 min)
.venv/bin/python3 -m pytest tests/greedy tests/stable_ac tests/analysis -q --runslow  # before any push or result claim (~5–8 min)
```
...
### Stable AC / CoV (AK(3), `experiments/stable_ac/cov/ak3/`)
...
- **A derived file that a verifier hard-fails on is part of the state — write it on the same cadence as the thing it describes, never only at clean shutdown.** covmeet's `summary.json` was written only in `run()`'s `finally:`, which SIGKILL skips, so a preempted 25 h vast.ai session left a 3-minute smoke summary (28,224 expanded) beside a 9,336,325-expanded snapshot — and because `verify_covmeet` fails on the disagreement and `report_covmeet` gates on the verifier, a 14.6M-orbit store that passed every real check was unreportable. Snapshot first, summary second (the write is ms against minutes). And never bump the identity tag as part of a durability fix: `ENGINE_TAG` would have made `load()` reject the very snapshot being rescued. [[TRAP]](experiments/lessons/summary-written-only-in-finally.md)
...
- An early-stop threshold on the same scale as the inputs silently skips whatever already satisfies it — `--stop-mu 13` gave AK(3) (μ_in = 13) one rung and a 2-orbit row that reads like inertness; rerun with the rule lowered it reaches 1,508 orbits and still nothing below 13. A low work-count is a stopping artifact until proven otherwise. [[TRAP]](experiments/lessons/stop-threshold-at-the-boundary-skips-the-boundary-case.md)
- Never concatenate CoV z-chains recorded against different representatives — CoV is representative-sensitive, so multi-generation chains junction at canonical reps via a Thm-3 automorphism and must be verified segment by segment; and couple a verification script to its commit with `&&`, never adjacency. [[TRAP]](experiments/lessons/cov-chains-junction-at-canonical-reps.md)
- A rung-local beam keeps only *this rung's* top-K, so it expands a moat-rim orbit once and abandons it — the front drifts up the cone and more orbits buy worse states. A global best-first heap over the open set pops in μ order and closes the ball instead: on `aca_39`, 5,831 of 22,224 orbits expanded ⇒ **no orbit below μ 22 anywhere in the μ ≤ 34 ball** (start 19), which a beam can never claim. Closure holds only for routes staying under the popped μ. [[WORKS]](experiments/lessons/rung-local-beam-abandons-the-low-shell.md)
- **Depth is not the knob on a rung-local beam** — plot the rung at which each result was *first* achieved before buying more rungs: a 256-rung/beam-64 ladder (347 CPU-h, 5.58M orbits) had every floor by rung 9, ran 98% of its rungs after its last gain, and bought one new descender over the r8 census. Report a plateau, never a wall. [[TRAP]](experiments/lessons/beam-ladder-saturates-by-rung-nine.md)
- **Before reading a treatment arm, check the control can move.** The orbit→greedy pilot returned 0 flips over 2,886 searches — and every control reduced `18 -> 18`: at budget 1,000 on `aca_124` nothing shows progress either way, so the null was under-powered by construction, not evidence. Say which claim a local cap makes untestable instead of shipping the number. [[TRAP]](experiments/lessons/control-with-no-dynamic-range.md) · [the pilot](results/stable_ac/orbit_greedy/ORBIT_GREEDY_PILOT.md)
- **An escape claim needs the cost of NOT doing the thing, at the budget where the untransformed route actually succeeds — not the budget where it was last seen to fail.** All eight CoV-resistant rows escape at budget 20,000 (cheapest start 14.4k nodes), which reads as 4.2×–18.9× over the plain greedy — until you price it: the oracle cost 2.2M nodes per presentation to find, the runnable blind restart wins 2.6×–3.4× on bin 9 and is a coin-flip on bin 8, and the tuned ordering beats both on all eight. Report oracle and runnable as separate columns, and stratify before pooling. **And sweep the collection budget B before quoting any of it** — a search at B is the first B pops of a longer one, so re-scoring every row at every smaller B costs zero new search, and doing it showed 20,000 is the *most favourable* B on half the rows (at 15,000 the restart loses on six of eight). A ratio that improves on the harder rows is the denominator moving until proven otherwise. [[TRAP]](experiments/lessons/price-the-untransformed-route.md) · [the run](results/stable_ac/cov/allcov_escape/ESCAPE.md)
...
- **When the winning configuration changes, re-sweep the knobs tuned for the old one — and include the knob's *off* setting as a control.** The endgame threshold 16 was chosen at budget 500 for a lean `L+8K` ordering and then carried into every later config unexamined; re-swept with an off arm it turned out **inert** for the multi-feature winner (19/24 either way) and worth +3..+5 only for lean knot-only climbs. The carried knob wasn't wrong, it was unnecessary — and it made the shipped recommendation more complex than the evidence. [[TRAP]](experiments/lessons/retune-knobs-when-the-winner-changes.md)

- **Alternative starts are worth running, never worth dividing a budget for.** Relabels supply most unsolved→solved flips *at full depth*, and no single ordering dominates (best single 19/24, union over all orderings 23/24) — but splitting a fixed budget across 8 relabels loses monotonically (19→14→8→3), because the mean number of restarts tried before a solve is 1.0: the alternates never fire. Measure which member produced the solve before believing in a portfolio. [[TRAP]](experiments/lessons/diversity-only-pays-at-full-depth.md)

- **A split is only "held out" from the stage that actually chose the thing.** `splits_aut.json` was verifiably automorphism-disjoint from `aut_train` — but the recommended weights were selected on the *stratified* slice, which contained 4 of the 7 "held-out" rows, turning 1/6→6/6 into an honest 1/3→3/3. State every published number as a **pair** (selected on X, evaluated on Y); "held out" alone is ambiguous once a project has two splits. An adversarial re-derivation of the headline from raw data beat a 26th experiment. [[TRAP]](experiments/lessons/held-out-means-held-out-from-selection.md)
...
- **Run local searches serially — one config at a time, in-process.** A reaped parent orphans its pool; eighteen such workers took the host 22 GB into swap and the user's editor read as 75 GB (its real RSS was 0.82 GB — macOS bills compression to the victim, so measure the accused app's own RSS first). `ps aux | grep python3` finds none of them: Homebrew's binary is capitalised `Python`. Sort by RSS instead. [[TRAP]](experiments/lessons/orphaned-pool-workers-exhaust-the-host.md)
- Interrupting a notebook cell kills only the parent — spawned workers survive, and a re-run double-computes every remaining row. A worker fleet needs a flock claim on its jsonl (live holder ⇒ orphan ⇒ kill), and a progress counter over an append-only file must count unique row keys, never lines. [[TRAP]](experiments/lessons/orphaned-workers-double-compute.md)
...
- A file under `results/` is data *and* somebody's test fixture — `grep -rn "$(basename FILE)" tests/ experiments/` before moving or deleting one; two commits that didn't left `test_verify_results.py` red for days, and that stale red masked a `verify_results.main` crash on its own default argument. When a whole test file is red, check the parent branch before blaming your change. [[TRAP]](experiments/lessons/results-path-is-a-test-fixture.md)
...
- The budget ceiling caps the *search*, not the machinery: any structural threshold the ceiling can't reach (chunk size, reservation floor, resize trigger) gets a test that drags the threshold below the ceiling — monkeypatch the constant, never raise the budget. [[WORKS]](experiments/lessons/budget-capped-tests-miss-structural-thresholds.md)
...
- **Every notebook runner ships the standard heartbeat — TIME-based (60 s in-search beat with instantaneous pops/s + ~5 min cumulative done/solved/ETA line), never event-based (a slow CPU must show as a falling rate, not silence) — AND the restart contract (Restart → Run All continues on an already-open notebook: UPDATE_REPO reset, module purge, seed_from_drive, .py-only hotfixes) — user directive, both mandatory.** [[WORKS]](experiments/lessons/notebook-heartbeat-and-restart-contract.md)
...
### Shell
- A repeated `.gitignore` entry after a negation is **not** a duplicate — `!ppo_checkpoints/610model/**` re-includes `.DS_Store`, so only the later copy ignores it; `git check-ignore -v` names the deciding rule, so run it before and after any de-duplication. [[TRAP]](experiments/lessons/gitignore-duplicate-was-load-bearing.md)
